...
http_conf="/opt/bizBrainz/templates/nginx-app-http.conf.template.sh"
https_conf="/opt/bizBrainz/templates/nginx-app-https.conf.template.sh"
ssl_conf_path="/bizBrainz-stacks/data/certificate/conf"
...
if [[ -z "${BIZBRAINZ_ALLOWED_FRAME_ANCESTORS-}" ]]; then
  # https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy/frame-ancestors
  export BIZBRAINZ_ALLOWED_FRAME_ANCESTORS="'self'"
else
  ...
  export BIZBRAINZ_ALLOWED_FRAME_ANCESTORS="${BIZBRAINZ_ALLOWED_FRAME_ANCESTORS%;*}"
fi

if [[ -z "${BIZBRAINZ_DISABLE_IFRAME_WIDGET_SANDBOX-}" ]]; then
  # For backwards compatibility, if this is not set to anything, we default to no sandbox for iframe widgets.
  export BIZBRAINZ_DISABLE_IFRAME_WIDGET_SANDBOX="true"
fi
...
if [[ -n ${BIZBRAINZ_CUSTOM_DOMAIN-} ]] && [[ -z ${DYNO-} ]]; then
  APP_TEMPLATE="$https_conf"
  if ! [[ -e "/etc/letsencrypt/live/$BIZBRAINZ_CUSTOM_DOMAIN" ]]; then
    source "/opt/bizBrainz/init_ssl_cert.sh"
    if ! init_ssl_cert "$BIZBRAINZ_CUSTOM_DOMAIN"; then
...
bash "$APP_TEMPLATE" "${BIZBRAINZ_CUSTOM_DOMAIN-}" > /etc/nginx/sites-available/default

index_html_served=/opt/bizBrainz/editor/index.html
index_html_original=/opt/bizBrainz/index.html.original
