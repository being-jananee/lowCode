...
import com.bizBrainz.external.helpers.BizbrainzBeanUtils;
...
import com.bizBrainz.server.dtos.EmailTokenDTO;
...
import com.bizBrainz.server.services.TenantService;
...
public class UserServiceCEImpl extends BaseService<UserRepository, User, String> implements UserServiceCE {
    ...
    public Mono<User> switchCurrentWorkspace(String workspaceId) {
        if (workspaceId == null || workspaceId.isEmpty()) {
            return Mono.error(new BizbrainzException(BizbrainzError.INVALID_PARAMETER, "workspaceId"));
        }
        return sessionUserService.getCurrentUser()
                ...
                .flatMap(user -> {
                    ...
                    if (workspaceIds == null || workspaceIds.isEmpty()) {
                        return Mono.error(new BizbrainzException(BizbrainzError.USER_DOESNT_BELONG_ANY_WORKSPACE, user.getId()));
                    }
                    ...
                    return Mono.error(new BizbrainzException(BizbrainzError.USER_DOESNT_BELONG_TO_WORKSPACE, user.getId(), workspaceId));
                });
    }
    ...
    public Mono<Boolean> forgotPasswordTokenGenerate(ResetUserPasswordDTO resetUserPasswordDTO) {
        if (resetUserPasswordDTO.getEmail() == null
            ...
            return Mono.error(new BizbrainzException(BizbrainzError.INVALID_PARAMETER, FieldName.EMAIL));
        }
        ...
        return repository.findByEmail(email)
                ...
                .switchIfEmpty(Mono.error(new BizbrainzException(BizbrainzError.NO_RESOURCE_FOUND, FieldName.USER, email)))
                .flatMap(user -> {
    ...
    private void validateResetLimit(PasswordResetToken resetToken) {
        if (resetToken.getRequestCount() >= 3) {
            ...
            if (l >= 24) { // ok, reset the counter
                ...
                throw new BizbrainzException(BizbrainzError.TOO_MANY_REQUESTS);
            }
        } else {
    ...
    public Mono<Boolean> verifyPasswordResetToken(String encryptedToken) {
        ...
        try {
            ...
            return Mono.error(new BizbrainzException(BizbrainzError.INVALID_PARAMETER, FieldName.TOKEN));
        }

        return passwordResetTokenRepository
                .findByEmail(emailTokenDTO.getEmail())
                .switchIfEmpty(Mono.error(new BizbrainzException(BizbrainzError.INVALID_PASSWORD_RESET)))
    ...
    public Mono<Boolean> resetPasswordAfterForgotPassword(String encryptedToken, User user) {
        ...
        return passwordResetTokenRepository
                ...
                .map(passwordResetToken -> {
                    ...
                    if (!matches) {
                        throw new BizbrainzException(BizbrainzError.GENERIC_BAD_REQUEST, FieldName.TOKEN);
                    } else {
                ...
                .flatMap(emailAddress -> repository
                        ...
                        .switchIfEmpty(Mono.error(new BizbrainzException(BizbrainzError.NO_RESOURCE_FOUND, FieldName.USER, emailAddress)))
                        .flatMap(userFromDb -> {
                            if (!ValidationUtils.validateLoginPassword(user.getPassword())) {
                                return Mono.error(new BizbrainzException(
                                        BizbrainzError.INVALID_PASSWORD_LENGTH, LOGIN_PASSWORD_MIN_LENGTH, LOGIN_PASSWORD_MAX_LENGTH)
                                );
                            }
                            ...
                            return passwordResetTokenRepository
                                    ...
                                    .switchIfEmpty(Mono.error(new BizbrainzException(
                                            BizbrainzError.NO_RESOURCE_FOUND, FieldName.TOKEN, emailTokenDTO.getToken()
                                    )))
    ...
    public Mono<UserSignupDTO> createUserAndSendEmail(User user, String originHeader) {
        ...
        if (originHeader == null || originHeader.isBlank()) {
            ...
            originHeader = Bizbrainz.DEFAULT_ORIGIN_HEADER;
        }
        ...
        if (LoginSource.FORM.equals(user.getSource())) {
            if (user.getPassword() == null || user.getPassword().isBlank()) {
                return Mono.error(new BizbrainzException(BizbrainzError.INVALID_CREDENTIALS));
            }
        ...
        return repository.findByCaseInsensitiveEmail(user.getUsername())
                .flatMap(savedUser -> {
                    ...
                    return Mono.error(new BizbrainzException(BizbrainzError.USER_ALREADY_EXISTS_SIGNUP, savedUser.getUsername()));
                })
                .switchIfEmpty(Mono.defer(() -> {
    ...
    private Mono<User> signupIfAllowed(User user) {
        ...
        if (!commonConfig.getAdminEmails().contains(user.getEmail())) {
            ...
            if (commonConfig.isSignupDisabled()) {
                ...
                return Mono.error(new BizbrainzException(BizbrainzError.SIGNUP_DISABLED, user.getUsername()));
            }
    ...
    public Mono<User> sendWelcomeEmail(User user, String originHeader) {
        ...
        return emailSender
                .sendMail(user.getEmail(), "Welcome to Bizbrainz", WELCOME_USER_EMAIL_TEMPLATE, params)
                .thenReturn(user)
                .onErrorResume(error -> {
    ...
    public Mono<User> update(String id, User userUpdate) {
        Mono<User> userFromRepository = repository.findById(id, MANAGE_USERS)
                .switchIfEmpty(Mono.error(new BizbrainzException(BizbrainzError.NO_RESOURCE_FOUND, FieldName.USER, id)));
        ...
        return userFromRepository
                .map(existingUser -> {
                    BizbrainzBeanUtils.copyNewFieldValuesIntoOldObject(userUpdate, existingUser);
                    return existingUser;
                })
                .flatMap(repository::save)
    ...
    public Mono<List<User>> inviteUsers(InviteUsersDTO inviteUsersDTO, String originHeader) {
        ...
        if (CollectionUtils.isEmpty(originalUsernames)) {
            return Mono.error(new BizbrainzException(BizbrainzError.INVALID_PARAMETER, FieldName.USERNAMES));
        }

        if (!StringUtils.hasText(inviteUsersDTO.getPermissionGroupId())) {
            return Mono.error(new BizbrainzException(BizbrainzError.INVALID_PARAMETER, FieldName.ROLE));
        }
        ...
        Mono<PermissionGroup> permissionGroupMono = permissionGroupService.getById(inviteUsersDTO.getPermissionGroupId(), AclPermission.ASSIGN_PERMISSION_GROUPS)
                ...
                .switchIfEmpty(Mono.error(new BizbrainzException(BizbrainzError.NO_RESOURCE_FOUND, FieldName.ROLE)))
                .cache();
                    ...
                    return repository.findByEmail(username)
                            .flatMap(existingUser -> {
                                ...
                                Mono<Boolean> emailMono = emailSender.sendMail(existingUser.getEmail(),
                                        "Bizbrainz: You have been added to a new workspace",
    ...
    private Mono<? extends User> createNewUserAndSendInviteEmail(String email, String originHeader,
        ...
        return userCreate(newUser, isAdminUser)
                .flatMap(createdUser -> {
                    ...
                    Mono<Boolean> emailMono = emailSender.sendMail(createdUser.getEmail(), "Invite for Bizbrainz", INVITE_USER_EMAIL_TEMPLATE, params);
    ...
    public Flux<User> get(MultiValueMap<String, String> params) {
        ...
        return Flux.error(new BizbrainzException(BizbrainzError.UNSUPPORTED_OPERATION));
    }
