...
import com.bizBrainz.server.dtos.UserSignupRequestDTO;
...
import static com.bizBrainz.server.constants.Bizbrainz.DEFAULT_ORIGIN_HEADER;
...
import static com.bizBrainz.server.helpers.RedirectHelper.REDIRECT_URL_QUERY_PARAM;
...
import static com.bizBrainz.server.helpers.ValidationUtils.validateEmail;
import static com.bizBrainz.server.helpers.ValidationUtils.validateLoginPassword;
...
public class UserSignupCEImpl implements UserSignupCE {
    ...
    public Mono<User> signupAndLogin(User user, ServerWebExchange exchange) {
        ...
        return Mono
                ...
                .switchIfEmpty(Mono.error(new BizbrainzException(BizbrainzError.INTERNAL_SERVER_ERROR)))
                .flatMap(tuple -> {
    ...
    public Mono<Void> signupAndLoginFromFormData(ServerWebExchange exchange) {
        ...
        return captchaService.verify(recaptchaToken).flatMap(verified -> {
                    if (!Boolean.TRUE.equals(verified)) {
                        return Mono.error(new BizbrainzException(BizbrainzError.GOOGLE_RECAPTCHA_FAILED));
                    }
