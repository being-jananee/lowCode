...
{
  echo "---------------------------------------------------------------------------------------"
  echo "Use this script to run a local instance of Bizbrainz on port 80."
...
popd
docker build -t bizBrainz/bizBrainz-ce:local-testing . > /dev/null && pretty_print "Docker image build successful. Triggering run now ..."

(docker stop bizBrainz || true) && (docker rm bizBrainz || true)
docker run -d --name bizBrainz -p 80:80 -v "$PWD/stacks:/bizBrainz-stacks" bizBrainz/bizBrainz-ce:local-testing && sleep 15 && pretty_print "Local instance is up! Open Bizbrainz at http://localhost! "