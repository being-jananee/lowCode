import {
  ...
  PLATFORM_ERROR,
  Severity,
} from "entities/BizbrainzConsole";
...
import { AnyReduxAction } from "@bizBrainz/constants/ReduxActionConstants";
    ...
    if (isWidget(entity) || isAction(entity) || isJSAction(entity)) {
      ...
      for (const { errors, isWarning, key: debuggerKey } of debuggerKeys) {
        ...
        if (errors.length) {
          ...
          BizbrainzConsole.deleteError(debuggerKey);
        }
      }
    }
  }
...
export function* handleJSFunctionExecutionErrorLog(
  ...
  errors.length
    ...
    : BizbrainzConsole.deleteError(`${collectionId}-${action.id}`);
}
