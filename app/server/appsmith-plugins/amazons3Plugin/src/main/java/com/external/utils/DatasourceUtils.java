...
import static com.bizBrainz.external.exceptions.pluginExceptions.BizbrainzPluginError.PLUGIN_ERROR;
...
public class DatasourceUtils {
    ...
    /**
     * Example endpoint : bizBrainz-test-storage-2.de-fra1.upcloudobjects.com
    ...
    public enum S3ServiceProvider {
        ...
        public static S3ServiceProvider fromString(String name) throws BizbrainzPluginException {
            ...
            throw new BizbrainzPluginException(PLUGIN_ERROR, "Bizbrainz S3 plugin service has " +
                    "failed to identify the S3 service provider type. Please reach out to Bizbrainz customer support" +
                    " to resolve this");
        }
    }
     ...
     * @throws BizbrainzPluginException when (1) there is an error with parsing credentials (2) required
    ...
    public static AmazonS3ClientBuilder getS3ClientBuilder (DatasourceConfiguration datasourceConfiguration)
        ...
        try {
            ...
            throw new BizbrainzPluginException(
                    ...
                    "Bizbrainz server has encountered an error when parsing AWS credentials from datasource: "
                            + e.getMessage()
            );
        }
        ...
        if (properties == null
            ...
            throw new BizbrainzPluginException(
                    BizbrainzPluginError.PLUGIN_DATASOURCE_ARGUMENT_ERROR,
                    "Bizbrainz has failed to fetch the 'S3 Service Provider' field properties. Please reach out to" +
     ...
     * @throws BizbrainzPluginException when then endpoint URL does not match the expected regex pattern.
     */
    private static String getRegionFromEndpointPattern(String endpoint, String regex, int regionGroupIndex)
        ...
        if (!endpoint.matches(regex)) {
            throw new BizbrainzPluginException(BizbrainzPluginError.PLUGIN_DATASOURCE_ARGUMENT_ERROR, "Your S3 endpoint" +
        ...
        throw new BizbrainzPluginException(BizbrainzPluginError.PLUGIN_ERROR, "Your S3 endpoint URL seems to be " +
                "incorrect for the selected S3 service provider. Please contact Bizbrainz customer " +
