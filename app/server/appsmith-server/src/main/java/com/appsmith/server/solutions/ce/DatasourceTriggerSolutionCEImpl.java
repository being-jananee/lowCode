...
import com.bizBrainz.external.models.ClientDataDisplayType;
...
import static com.bizBrainz.server.constants.FieldName.DISPLAY_TYPE;
import static com.bizBrainz.server.constants.FieldName.REQUEST_TYPE;
...
public class DatasourceTriggerSolutionCEImpl implements DatasourceTriggerSolutionCE {
    ...
    public Mono<TriggerResultDTO> trigger(String datasourceId, TriggerRequestDTO triggerRequestDTO) {
        ...
        Mono<Datasource> datasourceMono = datasourceService.findById(datasourceId, READ_DATASOURCES)
                .switchIfEmpty(Mono.error(new BizbrainzException(BizbrainzError.INVALID_PARAMETER, "datasourceId")))
                .cache();
        ...
        if (triggerRequestDTO.getRequestType() == null) {
            return Mono.error(new BizbrainzException(BizbrainzError.INVALID_PARAMETER, REQUEST_TYPE));
        }
    ...
    private Mono<Set<String>> entitySelectorTriggerSolution(Datasource datasource, TriggerRequestDTO request) {
        if (request.getDisplayType() == null) {
            return Mono.error(new BizbrainzException(BizbrainzError.INVALID_PARAMETER, DISPLAY_TYPE));
        }
