...
function showHelp() {
  console.log(
    "\nUsage: bizBrainz <command> to interact with bizBrainz utils tool"
  );
  console.log("\nOptions:\r");
  ...
  console.log("\tbackup\t\t\tTake a backup of Bizbrainz instance.\r");
  console.log("\trestore\t\t\tRestore Bizbrainz instance from a backup.\r");
  ...
  await fsPromises
    ...
    .then((filenames) => {
      for (let filename of filenames) {
        if (filename.match(/^bizBrainz-backup-.*\.tar\.gz$/)) {
