...
import static com.bizBrainz.external.constants.DataType.BOOLEAN;
import static com.bizBrainz.external.constants.DataType.DOUBLE;
import static com.bizBrainz.external.constants.DataType.FLOAT;
import static com.bizBrainz.external.constants.DataType.INTEGER;
import static com.bizBrainz.external.constants.DataType.LONG;
import static com.bizBrainz.external.constants.DataType.STRING;
...
public class PostgresDataTypeUtils {
    ...
    public static class DataType {
         ...
         * dataTypeMapper which maps the postgres data types to Bizbrainz data types.
         */
    ...
    // Stores the mapping between postgres data types and bizBrainz data types
    public static Map dataTypeMapper;

    private static Map getDataTypeMapper() {
        if (dataTypeMapper == null) {
            dataTypeMapper = new HashMap<String, com.bizBrainz.external.constants.DataType>();
            ...
            dataTypeMapper.put(DATE, com.bizBrainz.external.constants.DataType.DATE);
            dataTypeMapper.put(TIME, com.bizBrainz.external.constants.DataType.TIME);
            dataTypeMapper.put(FLOAT8, DOUBLE);
            dataTypeMapper.put(TEXT, STRING);
            dataTypeMapper.put(INT, INTEGER);

            // Must ensure that all the declared postgres data types have a mapping to bizBrainz data types
    ...
    public static List<com.bizBrainz.external.constants.DataType> extractExplicitCasting(String query) {
        Matcher matcher = questionWithCastPattern.matcher(query);
        List<com.bizBrainz.external.constants.DataType> inputDataTypes = new ArrayList<>();
        ...
        while (matcher.find()) {
            ...
            if (prospectiveDataType != null) {
                ...
                if (dataType.getDataTypes().contains(dataTypeFromInput)) {
                    com.bizBrainz.external.constants.DataType bizBrainzDataType
                            = (com.bizBrainz.external.constants.DataType) getDataTypeMapper().get(dataTypeFromInput);
                    inputDataTypes.add(bizBrainzDataType);
                    continue;
                }
            }
