...
import com.bizBrainz.server.repositories.CollectionRepository;
...
public class CollectionServiceCEImpl extends BaseService<CollectionRepository, Collection, String> implements CollectionServiceCE {
    ...
    public Mono<ActionDTO> addSingleActionToCollection(String collectionId, ActionDTO action) {
        ...
        if (action.getId() == null) {
            return Mono.error(new BizbrainzException(BizbrainzError.INVALID_PARAMETER, FieldName.ACTION));
        }

        return repository
                .findById(collectionId)
                .switchIfEmpty(Mono.error(new BizbrainzException(BizbrainzError.INVALID_PARAMETER, FieldName.COLLECTION_ID)))
                .flatMap(collection1 -> {
        ...
        return repository
                ...
                .flatMap(tuple -> {
                    ...
                    if (actions == null || actions.isEmpty()) {
                        return Mono.error(new BizbrainzException(BizbrainzError.INVALID_PARAMETER, FieldName.ACTION_ID + " or " + FieldName.COLLECTION_ID));
                    }
