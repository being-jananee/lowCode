...
import static com.bizBrainz.server.constants.FieldName.EMAIL;
import static com.bizBrainz.server.constants.FieldName.VIEWER;
import static com.bizBrainz.server.constants.FieldName.WEBSITE;
import static com.bizBrainz.server.constants.FieldName.WORKSPACE_ADMINISTRATOR_DESCRIPTION;
import static com.bizBrainz.server.constants.FieldName.WORKSPACE_DEVELOPER_DESCRIPTION;
import static com.bizBrainz.server.constants.FieldName.WORKSPACE_VIEWER_DESCRIPTION;
import static com.bizBrainz.server.constants.PatternConstants.EMAIL_PATTERN;
import static com.bizBrainz.server.constants.PatternConstants.WEBSITE_PATTERN;
...
public class WorkspaceServiceCEImpl extends BaseService<WorkspaceRepository, Workspace, String>
    ...
    public Mono<Workspace> create(Workspace workspace, User user) {
        if (workspace == null) {
            return Mono.error(new BizbrainzException(BizbrainzError.INVALID_PARAMETER, FieldName.WORKSPACE));
        }
        ...
        return createWorkspaceAllowedMono
                .flatMap(isCreateWorkspaceAllowed -> {
                    if (!isCreateWorkspaceAllowed) {
                        return Mono.error(new BizbrainzException(BizbrainzError.ACTION_IS_NOT_AUTHORIZED, "Create workspace"));
                    }
    ...
    public Mono<Workspace> update(String id, Workspace resource) {
        ...
        Mono<Workspace> findWorkspaceMono = repository.findById(id, MANAGE_WORKSPACES)
                .switchIfEmpty(Mono.error(new BizbrainzException(BizbrainzError.NO_RESOURCE_FOUND, FieldName.WORKSPACE, id)))
                .cache();
        ...
        return updateDefaultGroups_thenReturnWorkspaceMono
                .map(workspaceFromDb -> {
                    BizbrainzBeanUtils.copyNewFieldValuesIntoOldObject(resource, workspaceFromDb);
    ...
    public Mono<Workspace> deleteLogo(String workspaceId) {
        return repository
                ...
                .flatMap(workspace -> {
                    final String prevAssetId = workspace.getLogoAssetId();
                    if (prevAssetId == null) {
                        return Mono.error(new BizbrainzException(BizbrainzError.NO_RESOURCE_FOUND, FieldName.ASSET, prevAssetId));
                    }
                    workspace.setLogoAssetId(null);
                    return assetRepository.findById(prevAssetId)
                            .switchIfEmpty(Mono.error(new BizbrainzException(BizbrainzError.NO_RESOURCE_FOUND, FieldName.ASSET, prevAssetId)))
            ...
            if (appCount == 0) { // no application found under this workspace
                ...
                return repository.findById(workspaceId, MANAGE_WORKSPACES)
                        .switchIfEmpty(Mono.error(new BizbrainzException(
                                BizbrainzError.NO_RESOURCE_FOUND, FieldName.WORKSPACE, workspaceId
                        )))
    ...
    private void validateIncomingWorkspace(Workspace workspace){
        if (StringUtils.hasLength(workspace.getEmail()) && ! Pattern.matches(EMAIL_PATTERN, workspace.getEmail())) {
            throw new BizbrainzException(BizbrainzError.INVALID_PARAMETER, EMAIL);
        }
        if (StringUtils.hasLength(workspace.getWebsite()) && ! Pattern.matches(WEBSITE_PATTERN, workspace.getWebsite())) {
            throw new BizbrainzException(BizbrainzError.INVALID_PARAMETER, WEBSITE);
        }
    }
