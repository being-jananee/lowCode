...
function export_database() {
  ...
  const cmd = `mongodump --uri='${process.env.BIZBRAINZ_MONGODB_URI}' --archive='${Constants.BACKUP_PATH}/${Constants.DUMP_FILE_NAME}' --gzip`;
  shell.exec(cmd);
...
function run() {
  ...
  try {
    ...
    shell.echo();
    shell.echo(
      'Please remember to also copy BIZBRAINZ_ENCRYPTION_SALT and BIZBRAINZ_ENCRYPTION_PASSWORD variables from the docker.env file to the target instance where you intend to import this database dump.',
    );
    shell.echo();
    shell.echo('++++++++++++++++++++++++++++++++++++++++++++++\033[0m');
    shell.echo();
