...
import com.bizBrainz.external.models.DynamicBinding;
...
import static com.bizBrainz.external.helpers.BizbrainzBeanUtils.getBeanPropertyValues;
import static com.bizBrainz.external.helpers.BizbrainzBeanUtils.isDomainModel;
import static com.bizBrainz.external.helpers.SmartSubstitutionHelper.BIZBRAINZ_SUBSTITUTION_PLACEHOLDER;
...
public class MustacheHelper {
    ...
    /**
     * Bizbrainz smart replacement : The regex pattern below looks for '?' or "?". This pattern is later replaced with ?
    ...
    /**
     * Bizbrainz smart replacement with placeholder : The regex pattern below looks for `BIZBRAINZ_SUBSTITUTION_PLACEHOLDER`
     * surrounded by quotes. This pattern is later replaced with just BIZBRAINZ_SUBSTITUTION_PLACEHOLDER to fit the requirements
     * of JSON smart replacement aka trim the quotes present.
     */
    private static String regexPlaceholderTrimming = "([\"']" + BIZBRAINZ_SUBSTITUTION_PLACEHOLDER + "[\"'])";
    ...
    public static Set<String> extractMustacheKeys(String template) {
        ...
        for (String token : tokenize(template)) {
            if (token.startsWith("{{") && token.endsWith("}}")) {
                ...
                // up a value. Addresses https://www.notion.so/bizBrainz/Bindings-with-a-space-at-the-start-fail-to-execute-properly-in-the-API-pane-2eb65d5c6064466b9ef059fa01ef3261
    ...
    public static Set<String> extractMustacheKeysFromFields(Object object) {
        ...
        while (!processQueue.isEmpty()) {
            ...
            if (isDomainModel(obj.getClass())) {
                // Go deeper *only* if the property belongs to Bizbrainz's models, and both the source and target
                // values are not null.
     ...
     * - If object is an Bizbrainz domain object then iterate over all fields of the object and render field values
     * for each of them.
    ...
    public static String replaceMustacheWithPlaceholder(String query, List<String> mustacheBindings) {
        return replaceMustacheUsingPatterns(query, BIZBRAINZ_SUBSTITUTION_PLACEHOLDER, mustacheBindings,
                placeholderTrimmingPattern, BIZBRAINZ_SUBSTITUTION_PLACEHOLDER);
    }
