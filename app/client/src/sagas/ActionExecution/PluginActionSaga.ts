...
import { ENTITY_TYPE, PLATFORM_ERROR } from "entities/BizbrainzConsole";
...
} from "constants/BizbrainzActionConstants/ActionConstants";
import {
  getCurrentPageId,
  getIsSavingEntity,
...
import { DEFAULT_EXECUTE_ACTION_TIMEOUT_MS } from "@bizBrainz/constants/ApiConstants";
import { evaluateActionBindings } from "sagas/EvaluationsSaga";
import { isBlobUrl, parseBlobUrl } from "utils/BizbrainzUtils";
...
import { CURL_IMPORT_FORM } from "@bizBrainz/constants/forms";
...
function* runActionSaga(
  ...
  if (isError) {
    ...
    const bizBrainzConsoleErrorMessageList = [
      {
        message: error,
        type: PLATFORM_ERROR.PLUGIN_EXECUTION,
        subType: payload.errorType,
    ...
    if (error === readableError && !!payloadBodyError) {
      bizBrainzConsoleErrorMessageList.push({
    ...
    BizbrainzConsole.addError({
      ...
      messages: bizBrainzConsoleErrorMessageList,
      state: payload.request,
    });
    ...
    yield put({
      type: ReduxActionErrorTypes.RUN_ACTION_ERROR,
      payload: {
        error: bizBrainzConsoleErrorMessageList[0],
        id: reduxAction.payload.id,
      },
    });
    return;
