...
@Slf4j
public abstract class BaseService<R extends BaseRepository<T, ID> & BizbrainzRepository<T>, T extends BaseDomain, ID extends Serializable>
    ...
    public Mono<T> getById(ID id) {
        ...
        return repository.findById(id)
                .switchIfEmpty(Mono.error(new BizbrainzException(BizbrainzError.NO_RESOURCE_FOUND, "resource", id)));
    }
    ...
    protected Mono<T> validateObject(T obj) {
        return Mono.just(obj)
                ...
                .flatMap(constraint -> {
                    ...
                    return Mono.error(new BizbrainzException(BizbrainzError.INVALID_PARAMETER, constraint.stream().findFirst().get().getPropertyPath()));
                });
    }
