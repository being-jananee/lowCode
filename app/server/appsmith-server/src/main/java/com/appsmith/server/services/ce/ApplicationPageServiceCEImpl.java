...
import com.bizBrainz.external.helpers.BizbrainzEventContextType;
...
import com.bizBrainz.server.dtos.ApplicationPagesDTO;
import com.bizBrainz.server.dtos.PageDTO;
import com.bizBrainz.server.dtos.PageNameIdDTO;
...
import com.bizBrainz.server.migrations.ApplicationVersion;
...
public class ApplicationPageServiceCEImpl implements ApplicationPageServiceCE {
    ...
    public Mono<PageDTO> createPage(PageDTO page) {
        ...
        Mono<Application> applicationMono = applicationService.findById(page.getApplicationId(), AclPermission.MANAGE_APPLICATIONS)
                .switchIfEmpty(Mono.error(new BizbrainzException(BizbrainzError.NO_RESOURCE_FOUND, FieldName.APPLICATION, page.getApplicationId())))
                .cache();
    ...
    public Mono<UpdateResult> addPageToApplication(Application application, PageDTO page, Boolean isDefault) {
        ...
        if(isDuplicatePage(application, page.getId())) {
            ...
            return Mono.error(new BizbrainzException(BizbrainzError.DUPLICATE_KEY, "Page already exists with id "+page.getId()));
        }
    ...
    public Mono<PageDTO> getPage(String pageId, boolean viewMode) {
        ...
        return newPageService.findPageById(pageId, permission, viewMode)
                ...
                .switchIfEmpty(Mono.error(new BizbrainzException(BizbrainzError.ACL_NO_RESOURCE_FOUND, FieldName.PAGE, pageId)));
    }
    ...
    public Mono<PageDTO> getPageByName(String applicationName, String pageName, boolean viewMode) {
        ...
        return applicationService
                ...
                .switchIfEmpty(Mono.error(new BizbrainzException(BizbrainzError.ACL_NO_RESOURCE_FOUND, FieldName.PAGE + " by application name", applicationName)))
                ...
                .switchIfEmpty(Mono.error(new BizbrainzException(BizbrainzError.ACL_NO_RESOURCE_FOUND, FieldName.PAGE + " by page name", pageName)));
    }
    ...
    public Mono<Application> makePageDefault(String applicationId, String pageId) {
        ...
        return newPageService.findPageById(pageId, AclPermission.MANAGE_PAGES, viewMode)
                ...
                .flatMap(page -> {
                    ...
                    return Mono.error(new BizbrainzException(BizbrainzError.PAGE_DOESNT_BELONG_TO_APPLICATION, page.getName(), applicationId));
                })
                .then(applicationService.findById(applicationId, MANAGE_APPLICATIONS))
                .switchIfEmpty(Mono.error(new BizbrainzException(BizbrainzError.NO_RESOURCE_FOUND, FieldName.APPLICATION, applicationId)))
                .flatMap(application ->
    ...
    public Mono<Application> createApplication(Application application, String workspaceId) {
        ...
        if (workspaceId == null || workspaceId.isEmpty()) {
            return Mono.error(new BizbrainzException(BizbrainzError.INVALID_PARAMETER, FieldName.WORKSPACE_ID));
        }
        ...
        return userMono
                .flatMap(user -> {
                    Mono<Workspace> workspaceMono = workspaceRepository.findById(workspaceId, WORKSPACE_MANAGE_APPLICATIONS)
                            .switchIfEmpty(Mono.error(new BizbrainzException(BizbrainzError.NO_RESOURCE_FOUND, FieldName.WORKSPACE, workspaceId)));
    ...
    public Mono<Application> deleteApplication(String id) {
        ...
        Mono<Application> applicationMono = applicationRepository.findById(id, MANAGE_APPLICATIONS)
                .switchIfEmpty(Mono.error(new BizbrainzException(BizbrainzError.NO_RESOURCE_FOUND, FieldName.APPLICATION, id)))
                .cache();

        /* As part of git sync feature a new application will be created for each branch with reference to main application
         * feat/new-branch ----> new application in Bizbrainz
    ...
    public Mono<PageDTO> clonePage(String pageId) {
        ...
        return newPageService.findById(pageId, MANAGE_PAGES)
                .switchIfEmpty(Mono.error(new BizbrainzException(BizbrainzError.ACTION_IS_NOT_AUTHORIZED, "Clone Page")))
                .flatMap(page ->
    ...
    private Mono<PageDTO> clonePageGivenApplicationId(String pageId,
        ...
        Mono<PageDTO> sourcePageMono = newPageService.findPageById(pageId, MANAGE_PAGES, false)
                .switchIfEmpty(Mono.error(new BizbrainzException(BizbrainzError.NO_RESOURCE_FOUND, FieldName.PAGE, pageId)))
                ...
                .flatMap(clonedPage -> {
                    ...
                    return sourceActionFlux
                            .flatMap(action -> {
                                 ...
                                 */
                                BizbrainzEventContext eventContext = new BizbrainzEventContext(BizbrainzEventContextType.CLONE_PAGE);
    ...
    public Mono<PageDTO> deleteUnpublishedPage(String id) {
        ...
        return newPageService.findById(id, AclPermission.MANAGE_PAGES)
                .switchIfEmpty(Mono.error(new BizbrainzException(BizbrainzError.NO_RESOURCE_FOUND, FieldName.PAGE, id)))
                .flatMap(page -> {
    ...
    public Mono<Application> publish(String applicationId, boolean isPublishedManually) {
        ...
        Mono<List<NewPage>> publishApplicationAndPages = applicationMono
                ...
                .flatMap(applicationPage -> newPageService
                        ...
                        .switchIfEmpty(Mono.error(new BizbrainzException(BizbrainzError.NO_RESOURCE_FOUND, FieldName.PAGE, applicationPage.getId())))
                        .map(page -> {
    ...
    public Mono<ApplicationPagesDTO> reorderPage(String defaultAppId, String defaultPageId, Integer order, String branchName) {
        return newPageService.findByBranchNameAndDefaultPageId(branchName, defaultPageId, MANAGE_PAGES)
                .switchIfEmpty(Mono.error(new BizbrainzException(BizbrainzError.NO_RESOURCE_FOUND, FieldName.PAGE, defaultPageId)))
                .zipWhen(branchedPage -> applicationService.findById(branchedPage.getApplicationId(), MANAGE_APPLICATIONS)
                        .switchIfEmpty(Mono.error(new BizbrainzException(BizbrainzError.NO_RESOURCE_FOUND, FieldName.APPLICATION, defaultAppId))))
                .flatMap(tuple -> {
