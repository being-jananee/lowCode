...
import com.bizBrainz.server.constants.ApplicationConstants;
...
import com.bizBrainz.server.dtos.GitAuthDTO;
...
public class ApplicationServiceCEImpl extends BaseService<ApplicationRepository, Application, String> implements ApplicationServiceCE {
        ...
        if (application.getApplicationVersion() != null) {
            ...
            if (appVersion < ApplicationVersion.EARLIEST_VERSION || appVersion > ApplicationVersion.LATEST_VERSION) {
                return Mono.error(new BizbrainzException(
                        BizbrainzError.INVALID_PARAMETER,
                ...
                .flatMap(appId -> repository.updateById(appId, application, AclPermission.MANAGE_APPLICATIONS)
                        .onErrorResume(error -> {
                            if (error instanceof DuplicateKeyException) {
                                // Error message : E11000 duplicate key error collection: bizBrainz.application index:
                                ...
                                if (error.getCause().getMessage().contains("workspace_application_deleted_gitApplicationMetadata_compound_index")) {
                                    return Mono.error(
                                            new BizbrainzException(BizbrainzError.DUPLICATE_KEY_USER_ERROR, FieldName.APPLICATION, FieldName.NAME)
                                    );
                                }
                                return Mono.error(new BizbrainzException(BizbrainzError.DUPLICATE_KEY, error.getCause().getMessage()));
                            }
    ...
    public Mono<Application> changeViewAccess(String id, ApplicationAccessDTO applicationAccessDTO) {
        ...
        Mono<Application> updateApplicationMono = repository.findById(id, MAKE_PUBLIC_APPLICATIONS)
                .switchIfEmpty(Mono.error(new BizbrainzException(BizbrainzError.ACL_NO_RESOURCE_FOUND, FieldName.APPLICATION, id)))
    ...
    public Mono<GitAuth> createOrUpdateSshKeyPair(String applicationId, String keyType) {
        ...
        return repository.findById(applicationId, MANAGE_APPLICATIONS)
                .switchIfEmpty(Mono.error(
                        new BizbrainzException(BizbrainzError.NO_RESOURCE_FOUND, "application", applicationId)
                ))
                .flatMap(application -> {
                    GitApplicationMetadata gitData = application.getGitApplicationMetadata();
                    ...
                    if (StringUtils.isEmpty(gitData.getDefaultApplicationId())) {
                        throw new BizbrainzException(BizbrainzError.INVALID_GIT_CONFIGURATION,
    ...
    public Mono<GitAuthDTO> getSshKey(String applicationId) {
        return repository.findById(applicationId, MANAGE_APPLICATIONS)
                .switchIfEmpty(
                        Mono.error(new BizbrainzException(BizbrainzError.ACL_NO_RESOURCE_FOUND, FieldName.APPLICATION_ID, applicationId))
                )
                .flatMap(application -> {
                    GitApplicationMetadata gitData = application.getGitApplicationMetadata();
    ...
    public Mono<Application> findByBranchNameAndDefaultApplicationId(String branchName,
        ...
        if (StringUtils.isEmpty(branchName)) {
            return repository.findById(defaultApplicationId, aclPermission)
                    .switchIfEmpty(Mono.error(
                            new BizbrainzException(BizbrainzError.NO_RESOURCE_FOUND, FieldName.APPLICATION, defaultApplicationId))
                    );
        }
        return repository.getApplicationByGitBranchAndDefaultApplicationId(defaultApplicationId, branchName, aclPermission)
                .switchIfEmpty(Mono.error(
                        new BizbrainzException(BizbrainzError.NO_RESOURCE_FOUND, FieldName.APPLICATION, defaultApplicationId + "," + branchName))
                );
    }
    ...
    public Mono<Application> saveLastEditInformation(String applicationId) {
        ...
        /*
          ...
          by the updateById method of the BaseBizbrainzRepositoryImpl
         */
    ...
    public Mono<String> findBranchedApplicationId(String branchName, String defaultApplicationId, AclPermission permission) {
        if (StringUtils.isEmpty(branchName)) {
            if (StringUtils.isEmpty(defaultApplicationId)) {
                return Mono.error(new BizbrainzException(BizbrainzError.INVALID_PARAMETER, FieldName.APPLICATION_ID, defaultApplicationId));
            }
