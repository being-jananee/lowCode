...
interface BizbrainzLayoutConfigOption {
  name: string;
  type: SupportedLayouts;
  icon?: IconName;
}

export const BizbrainzDefaultLayout: AppLayoutConfig = {
  type: "FLUID",
};

const BizbrainzLayouts: BizbrainzLayoutConfigOption[] = [
  {
    name: "Fluid Width",
    type: "FLUID",
    icon: "fluid",
...
export function MainContainerLayoutControl() {
  ...
  const selectedIndex = useMemo(() => {
    return BizbrainzLayouts.findIndex(
      (each) => each.type === (appLayout?.type || BizbrainzDefaultLayout.type),
    );
  }, [appLayout]);
  ...
  return (
    <div className="space-y-2 t--layout-control-wrapper">
      ...
      >
        {BizbrainzLayouts.map((layoutOption: any, index: number) => {
          return (
            <TooltipComponent
              className="flex-grow"
              ...
              position={
                index === BizbrainzLayouts.length - 1 ? "bottom-right" : "bottom"
              }
            >
              <button
