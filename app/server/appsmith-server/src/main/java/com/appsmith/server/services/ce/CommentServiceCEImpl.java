...
import com.bizBrainz.server.helpers.CommentUtils;
import com.bizBrainz.server.helpers.PolicyUtils;
import com.bizBrainz.server.helpers.ResponseUtils;
import com.bizBrainz.server.helpers.TemplateUtils;
...
import static com.bizBrainz.server.constants.CommentConstants.BIZBRAINZ_BOT_NAME;
...
public class CommentServiceCEImpl extends BaseService<CommentRepository, Comment, String> implements CommentServiceCE {
        ...
        return Mono.zip(branchedAppIdMono, branchedPageIdMono, userMono)
                .flatMap(tuple -> {
                    ...
                    return threadRepository
                            ...
                            .switchIfEmpty(Mono.error(new BizbrainzException(BizbrainzError.ACL_NO_RESOURCE_FOUND, FieldName.COMMENT_THREAD, threadId)))
            ...
            return Mono.zip(
                    ...
                    applicationService
                            ...
                            .switchIfEmpty(Mono.error(new BizbrainzException(
                                    BizbrainzError.ACL_NO_RESOURCE_FOUND, FieldName.APPLICATION, applicationId)
                            )),
    ...
    public Mono<CommentThread> updateThread(String threadId, CommentThread commentThread, String originHeader) {
        ...
        return sessionUserService.getCurrentUser().flatMap(user -> {
                ...
                .switchIfEmpty(Mono.error(
                        new BizbrainzException(BizbrainzError.ACL_NO_RESOURCE_FOUND, "comment thread", threadId))
                )
                .flatMap(tuple -> {
    ...
    public Mono<List<CommentThread>> getThreadsByApplicationId(CommentThreadFilterDTO commentThreadFilterDTO) {
        return applicationService.findById(commentThreadFilterDTO.getApplicationId(), READ_APPLICATIONS)
                .switchIfEmpty(Mono.error(new BizbrainzException(
                        BizbrainzError.NO_RESOURCE_FOUND, FieldName.APPLICATION, commentThreadFilterDTO.getApplicationId()
                )))
    ...
    public Mono<Comment> deleteComment(String id) {
        return repository.findById(id, AclPermission.MANAGE_COMMENTS)
                .switchIfEmpty(Mono.error(new BizbrainzException(BizbrainzError.NO_RESOURCE_FOUND, FieldName.COMMENT, id)))
    ...
    private Mono<Comment> createBotComment(CommentThread commentThread, User user, CommentOnboardingState commentOnboardingState) {
        ...
        comment.setThreadId(commentThread.getId());
        comment.setAuthorName(BIZBRAINZ_BOT_NAME);
        comment.setAuthorUsername(BIZBRAINZ_BOT_USERNAME);
        comment.setApplicationId(commentThread.getApplicationId());
        comment.setWorkspaceId(commentThread.getWorkspaceId());
        ...
        botCommentParams.put("BizbrainzBotName", BIZBRAINZ_BOT_NAME);
        botCommentParams.put("BizbrainzBotUserName", BIZBRAINZ_BOT_USERNAME);
        ...
        try {
            if (commentOnboardingState == CommentOnboardingState.COMMENTED) {
                ...
                block.setEntityRanges(List.of(new Comment.Range(92, BIZBRAINZ_BOT_USERNAME.length(), 0)));

                Comment.EntityData entityData = new Comment.EntityData();
                entityData.setMention(new Comment.EntityData.Mention("bizBrainz", null));
