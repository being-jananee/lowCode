...
module.exports = (on, config) => {
  ...
  /**
   * Fallback to BIZBRAINZ_* env variables for Cypress.env if config.env doesn't already have it.
   * Note: BIZBRAINZ_* ENV vars have lower precedence than *all* methods mentioned in https://docs.cypress.io/guides/guides/environment-variables.html
   * Example #1:
   * process.env -> BIZBRAINZ_FOO=bar
   * cypress.json -> BIZBRAINZ_FOO=baz
   *
   * Cypress.env("BIZBRAINZ_FOO") // baz
   *
   * Example #2:
   * process.env -> BIZBRAINZ_FOO=bar
   * cypress.json -> BIZBRAINZ_FOO=
   *
   * Cypress.env("BIZBRAINZ_FOO") // <empty>
   */
  Object.keys(process.env).forEach((key) => {
    if (
      key.startsWith("BIZBRAINZ_") &&
   ...
   * Note: Value in .env file has the lowest precedence, even lower than BIZBRAINZ_* ENV vars.
   * Example:
   * .env -> BIZBRAINZ_FOO=bar
   * process.env -> BIZBRAINZ_FOO=
   *
   * Cypress.env("BIZBRAINZ_FOO") // <empty>
   */
  try {
