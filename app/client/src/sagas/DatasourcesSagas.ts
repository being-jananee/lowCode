...
} from "@bizBrainz/constants/forms";
import { validateResponse } from "./ErrorSagas";
import AnalyticsUtil from "utils/AnalyticsUtil";
import { getFormData } from "selectors/formSelectors";
import { getCurrentWorkspaceId } from "@bizBrainz/selectors/workspaceSelectors";
import { Variant } from "components/ads/common";
import { Toaster } from "components/ads/Toast";
import { getConfigInitialValues } from "components/formControls/utils";
import { setActionProperty } from "actions/pluginActionActions";
import { authorizeDatasourceWithBizbrainzToken } from "api/CloudServicesApi";
import {
  createMessage,
  DATASOURCE_CREATE,
  DATASOURCE_DELETE,
  DATASOURCE_UPDATE,
  DATASOURCE_VALID,
  OAUTH_BIZBRAINZ_TOKEN_NOT_FOUND,
  OAUTH_AUTHORIZATION_BIZBRAINZ_ERROR,
  OAUTH_AUTHORIZATION_FAILED,
  OAUTH_AUTHORIZATION_SUCCESSFUL,
} from "@bizBrainz/constants/messages";
import BizbrainzConsole from "utils/BizbrainzConsole";
import { ENTITY_TYPE } from "entities/BizbrainzConsole";
import localStorage from "utils/localStorage";
import log from "loglevel";
import { BIZBRAINZ_TOKEN_STORAGE_KEY } from "pages/Editor/SaaSEditor/constants";
import { checkAndGetPluginFormConfigsSaga } from "sagas/PluginSagas";
import { PluginType } from "entities/Action";
import LOG_TYPE from "entities/BizbrainzConsole/logtype";
...
function* redirectAuthorizationCodeSaga(
  ...
  if (pluginType === PluginType.API) {
    ...
    try {
      // Get an "bizBrainz token" from the server
      const response: ApiResponse<string> = yield OAuthApi.getBizbrainzToken(
        datasourceId,
        pageId,
        !!isImport,
      );
      ...
      if (validateResponse(response)) {
        const bizBrainzToken = response.data;
        // Save the token for later use once we come back from the auth flow
        localStorage.setItem(BIZBRAINZ_TOKEN_STORAGE_KEY, bizBrainzToken);
        // Redirect to the cloud services to authorise
        window.location.assign(
          authorizeDatasourceWithBizbrainzToken(bizBrainzToken),
        );
      }
    } catch (e) {
      Toaster.show({
...
function* getOAuthAccessTokenSaga(
  ...
  // get the saved bizBrainz token that started the auth request
  const bizBrainzToken = localStorage.getItem(BIZBRAINZ_TOKEN_STORAGE_KEY);
  if (!bizBrainzToken) {
    // Error out because auth token should been here
    log.error(OAUTH_BIZBRAINZ_TOKEN_NOT_FOUND);
    Toaster.show({
      text: OAUTH_AUTHORIZATION_BIZBRAINZ_ERROR,
      variant: Variant.danger,
    });
    return;
  }
  try {
    ...
    if (validateResponse(response)) {
      ...
      localStorage.removeItem(BIZBRAINZ_TOKEN_STORAGE_KEY);
    }
  } catch (e) {
    Toaster.show({
      text: OAUTH_AUTHORIZATION_FAILED,
