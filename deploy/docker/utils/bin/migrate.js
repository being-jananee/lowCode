...
const main = async (userServer, ipServer) => {
  ...
  if (answerConfirm.toLowerCase() === 'y') {
    const folderSsh = `/opt/bizBrainz/.ssh`;
    ...
    if (answerKey.toLowerCase() === 'y') {
      ...
      if (status.code === 0) {
        ...
        const installDir = readlineSync.question('Choose Installation Directory [bizBrainz]: ');

        let installAbsoluteDir = `/home/${userServer}/bizBrainz`;
        ...
        const resDocker = shell.exec(
          `ssh -i ${folderSsh}/id_rsa ${userServer}@${ipServer} 'bash -s ${installAbsoluteDir}' < /opt/bizBrainz/install_docker.sh`,
        );
        if (resDocker.code === 1) {
          process.kill(process.pid);
        }
        ...
        const resPull = shell.exec(
          `ssh -i ${folderSsh}/id_rsa ${userServer}@${ipServer} 'bash -s ${installAbsoluteDir}' < /opt/bizBrainz/pull_resource.sh`,
        );
        if (resPull.code === 1) {
          process.kill(process.pid);
        }
        ...
        shell.exec(
          `scp -i ${folderSsh}/id_rsa -r /bizBrainz-stacks/configuration ${userServer}@${ipServer}:${installAbsoluteDir}/stacks`,
        );
        shell.exec(
          `scp -i ${folderSsh}/id_rsa -r /bizBrainz-stacks/data/backup ${userServer}@${ipServer}:${installAbsoluteDir}/stacks/data`,
        );
        shell.exec(
          `scp -i ${folderSsh}/id_rsa -r /bizBrainz-stacks/letsencrypt ${userServer}@${ipServer}:${installAbsoluteDir}/stacks`,
        );
        shell.exec(`ssh -i ${folderSsh}/id_rsa ${userServer}@${ipServer} 'bash -s ${installAbsoluteDir}' < /opt/bizBrainz/start_app.sh`);
