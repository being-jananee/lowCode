...
function import_database() {
  ...
  const cmd = `mongorestore --uri='${process.env.BIZBRAINZ_MONGODB_URI}' --drop --archive='${Constants.RESTORE_PATH}/${Constants.DUMP_FILE_NAME}' --gzip`
  shell.exec(cmd)
...
const main = (forceOption) => {
  ...
  try {
    ...
    stop_application()
    const shellCmdResult = shell.exec(`mongo ${process.env.BIZBRAINZ_MONGODB_URI} --quiet --eval "db.getCollectionNames().length"`)
