...
# This function checks if the relevant ports required by Bizbrainz are available or not
...
check_ports_occupied() {
    ...
    if [[ -n $port_check_output ]]; then
        ...
        --data '{
            "userId": "'"$BIZBRAINZ_INSTALLATION_ID"'",
        ...
        echo "+++++++++++ ERROR ++++++++++++++++++++++"
        echo "Bizbrainz requires ports 80 & 443 to be open. Please shut down any other service(s) that may be running on these ports."
        echo "You can run bizBrainz on another port following this guide https://docs.bizBrainz.com/v/v1.2.1/troubleshooting-guide/deployment-errors"
        echo "++++++++++++++++++++++++++++++++++++++++"
        echo ""
        exit 1
    fi
...
echo_contact_support() {
    echo "Please contact <support@bizBrainz.com> with your OS details and version${1:-.}"
}
...
ask_telemetry() {
    echo ""
    echo "+++++++++++ IMPORTANT ++++++++++++++++++++++"
    echo -e "Thank you for installing bizBrainz! We want to be transparent and request that you share anonymous usage data with us."
    ...
    echo -e "You can read more about what information is collected in our documentation https://docs.bizBrainz.com/v/v1.2.1/setup/telemetry"
    echo -e ""
    if confirm y 'Would you like to share anonymous usage data and receive better support?'; then
        disable_telemetry="false"
    else
        disable_telemetry="true"
        echo "Please note that even though telemetry is disabled, your Bizbrainz server will connect to cloud to fetch release notes and to check for updates."
    fi
    echo "++++++++++++++++++++++++++++++++++++++++++++"
...
echo -e "👋 Thank you for trying out Bizbrainz! "
echo ""
...
BIZBRAINZ_INSTALLATION_ID=$(curl -s 'https://api64.ipify.org')
...
if [[ $desired_os -eq 0 ]];then
    echo ""
    echo "This script is currently meant to install Bizbrainz on Mac OS X, Ubuntu, Debian, Linux Mint, Red Hat, CentOS, SLES or openSUSE machines."
...
read -rp 'Create an Installation Directory: [bizBrainz]' install_dir
install_dir="${install_dir:-bizBrainz}"
if [[ $install_dir != /* ]]; then
...
echo "Installing Bizbrainz to '$install_dir'."
mkdir -p "$install_dir"
echo ""
...
if confirm y "Is this a fresh installation?"; then
   ...
   curl -s --location --request POST 'https://hook.integromat.com/dkwb6i52am93pi30ojeboktvj32iw0fa' \
    ...
    echo 'You are trying to connect to an existing bizBrainz database. Abort if you want to install bizBrainz using the default database'
    read -rp 'Enter your existing bizBrainz mongo db host: ' mongo_host
    read -rp 'Enter your existing bizBrainz mongo root user: ' mongo_root_user
    read -srp 'Enter your existing bizBrainz mongo password: ' mongo_root_password
    echo ""
    read -rp 'Enter your existing bizBrainz mongo database name: ' mongo_database
...
if test -f "$encryptionEnv"; then
    echo "CAUTION : This isn't your first time installing bizBrainz. Encryption password and salt already exist. Do you want to override this? NOTE: Overwriting the existing salt and password would lead to you losing access to sensitive information encrypted using the same"
...
if [[ "$setup_encryption" = "true" ]];then
    if [[ "$auto_generate_encryption" = "false" ]];then
        echo "Please enter the salt and password found in the encyption.env file of your previous bizBrainz installation "
...
if confirm n "Do you have a custom domain that you would like to link? (Only for cloud installations)"; then
    ...
    echo "https://docs.bizBrainz.com/v/v1.2.1/setup#custom-domains"
    echo "+++++++++++++++++++++++++++++++++++++++++++++++"
    echo ""
    ...
    if confirm y '(Your DNS records must be updated for us to proceed)'; then
        ...
        read -rp 'Enter the domain or subdomain on which you want to host bizBrainz (example.com / app.example.com): ' custom_domain
    fi
fi
...
(
    ...
    curl --remote-name-all --silent --show-error \
        https://raw.githubusercontent.com/bizBrainzorg/bizBrainz/master/deploy/template/docker-compose.yml.sh \
        https://raw.githubusercontent.com/bizBrainzorg/bizBrainz/master/deploy/template/mongo-init.js.sh \
        https://raw.githubusercontent.com/bizBrainzorg/bizBrainz/master/deploy/template/docker.env.sh \
        https://raw.githubusercontent.com/bizBrainzorg/bizBrainz/master/deploy/template/nginx_app.conf.sh \
        https://raw.githubusercontent.com/bizBrainzorg/bizBrainz/master/deploy/template/encryption.env.sh
)
...
if [[ $status_code -ne 401 ]]; then
    ...
    echo "Please read our troubleshooting guide https://docs.bizBrainz.com/v/v1.2.1/troubleshooting-guide/deployment-errors"
    ...
    echo ""
    echo "Please share your email to receive support & updates about bizBrainz!"
    read -rp 'Email: ' email
    curl -s --location --request POST 'https://hook.integromat.com/dkwb6i52am93pi30ojeboktvj32iw0fa' \
    --header 'Content-Type: text/plain' \
    --data '{
