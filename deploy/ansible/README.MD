...
This document will explain, to those unfamiliar with Ansible, how they can get an Ansible environment set-up quickly, with the end goal of deploying Bizbrainz.
...
# Simple Deployment Environment for Bizbrainz

So, now you've got Ansible installed, you can get ready to deploy Bizbrainz!
...
First you will need to clone the bizBrainz repository to your machine & move to the ansible playbook folder

```
$ git clone https://github.com/bizBrainzorg/bizBrainz.git
$ cd ./bizBrainz/deploy/ansible/bizBrainz_playbook
```
...
Now, with your editor, open the file and add the hostname or FQDN of the server(s) you want to deploy Bizbrainz to with the following pattern:

```
bizBrainz ansible_host={{ SERVER_HOST }} ansible_port={{ SERVER_PORT }} ansible_user={{ SERVER_USER }}
```
...
```
bizBrainz ansible_host={{ SERVER_HOST }} ansible_port={{ SERVER_PORT }} ansible_user={{ SERVER_USER }} ansible_ssh_private_key_file={{ SSH_PRIVATE_KEY_FILE }}
```

After you completed the above step then we're pretty much done with the inventory

## Setup your configuration vars for Bizbrainz

The next step is to setup necessary configuration for your app to run such as environment variable, domain name, etc.

First you need to open `bizBrainz-vars.yml` file with your editor.  
There are some variables that will need input from you to get the application start correctly

- `install_dir`: The absolute path of your app's installation folder on the server (required). Default: `~/bizBrainz`
...
```
$ ansible-playbook -i inventory bizBrainz-playbook.yml --extra-vars "@bizBrainz-vars.yml" --tags untagged
```

The command above will use the host information from the `inventory` file & feed your configuration vars from `bizBrainz-vars.yml` before running the playbook
