...
if [[ -e "/bizBrainz-stacks/ssl/fullchain.pem" ]] && [[ -e "/bizBrainz-stacks/ssl/privkey.pem" ]]; then
  SSL_CERT_PATH="/bizBrainz-stacks/ssl/fullchain.pem"
  SSL_KEY_PATH="/bizBrainz-stacks/ssl/privkey.pem"
fi
...
server {
  ...
  include /bizBrainz-stacks/data/certificate/conf/options-ssl-nginx.conf;
  ssl_dhparam /bizBrainz-stacks/data/certificate/conf/ssl-dhparams.pem;

  # https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy/frame-ancestors
  add_header Content-Security-Policy "frame-ancestors ${BIZBRAINZ_ALLOWED_FRAME_ANCESTORS-'self' *}";
  ...
  root /opt/bizBrainz/editor;
  index index.html;
  error_page 404 /;

  location /.well-known/acme-challenge/ {
    root /bizBrainz-stacks/data/certificate/certbot;
  }
