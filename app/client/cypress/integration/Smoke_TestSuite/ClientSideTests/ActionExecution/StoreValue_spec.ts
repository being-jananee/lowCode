...
describe("storeValue Action test", () => {
    ...
    const jsObjectBody = `export default {
      storeTest: () => {
        ...
        return Promise.all(values)
          .then(() => {
            showAlert(JSON.stringify(bizBrainz.store))
        })
          .catch((err) => {
  ...
  it("2. Bug 14827 : Accepts paths as keys and doesn't update paths in store but creates a new field with path as key", function() {
    ...
    const JS_OBJECT_BODY = `export default {
        ...
        await showAlert(JSON.stringify(bizBrainz.store.student));
        await storeValue("student.details.name", "Annah", false);
        await showAlert(bizBrainz.store.student.details.name);
        await showAlert(bizBrainz.store["student.details.name"]);
       },
       modifyStorePathTest: async ()=>{
        ...
        await showAlert(bizBrainz.store.student.details.isTopper.toString());
        await showAlert(bizBrainz.store["student.details.isTopper"].toString());
       }
       }
   `;
  ...
  it("3. Bug 14827 : Accepts paths as keys and doesn't update paths in store but creates a new field with path as key - object keys", function() {
    ...
    const JS_OBJECT_BODY = `export default {
      setStore: async () => {
        ...
        await showAlert(JSON.stringify(bizBrainz.store.test));
        await storeValue("test.two",{"b":2}, false);
        await showAlert(JSON.stringify(bizBrainz.store.test.two));
        await showAlert(JSON.stringify(bizBrainz.store["test.two"]));
      },
      showStore: () =>  {
        showAlert(JSON.stringify(bizBrainz.store.test));}
    }`;
