...
const baseOptions: { label: string; value: string }[] = [
  {
    ...
    value: BizbrainzFunction.none,
  },
  {
    label: createMessage(EXECUTE_A_QUERY),
    value: BizbrainzFunction.integration,
  },
  {
    label: createMessage(NAVIGATE_TO),
    value: BizbrainzFunction.navigateTo,
  },
  {
    label: createMessage(SHOW_MESSAGE),
    value: BizbrainzFunction.showAlert,
  },
  {
    label: createMessage(OPEN_MODAL),
    value: BizbrainzFunction.showModal,
  },
  {
    label: createMessage(CLOSE_MODAL),
    value: BizbrainzFunction.closeModal,
  },
  {
    label: createMessage(STORE_VALUE),
    value: BizbrainzFunction.storeValue,
  },
  {
    label: createMessage(DOWNLOAD),
    value: BizbrainzFunction.download,
  },
  {
    label: createMessage(COPY_TO_CLIPBOARD),
    value: BizbrainzFunction.copyToClipboard,
  },
  {
    label: createMessage(RESET_WIDGET),
    value: BizbrainzFunction.resetWidget,
  },
  {
    label: createMessage(SET_INTERVAL),
    value: BizbrainzFunction.setInterval,
  },
  {
    label: createMessage(CLEAR_INTERVAL),
    value: BizbrainzFunction.clearInterval,
  },
  {
    label: createMessage(GET_GEO_LOCATION),
    value: BizbrainzFunction.getGeolocation,
  },
  {
    label: createMessage(WATCH_GEO_LOCATION),
    value: BizbrainzFunction.watchGeolocation,
  },
  {
    label: createMessage(STOP_WATCH_GEO_LOCATION),
    value: BizbrainzFunction.stopWatchGeolocation,
  },
];
...
const getBaseOptions = (featureFlags: FeatureFlags) => {
  ...
  if (isJSEditorEnabled) {
    const jsOption = baseOptions.find(
      (option: any) => option.value === BizbrainzFunction.jsFunction,
    );
    if (!jsOption) {
      baseOptions.splice(2, 0, {
        label: createMessage(EXECUTE_JS_FUNCTION),
        value: BizbrainzFunction.jsFunction,
      });
    }
  }
  return baseOptions;
...
function getIntegrationOptionsWithChildren(
  ...
  const option = options.find(
    (option) => option.value === BizbrainzFunction.integration,
  );

  const jsOption = options.find(
    (option) => option.value === BizbrainzFunction.jsFunction,
  );
