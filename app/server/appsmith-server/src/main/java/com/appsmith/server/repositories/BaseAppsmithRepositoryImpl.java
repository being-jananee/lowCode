...
@Slf4j
public abstract class BaseBizbrainzRepositoryImpl<T extends BaseDomain> {
    ...
    @Autowired
    public BaseBizbrainzRepositoryImpl(ReactiveMongoOperations mongoOperations,
                                      MongoConverter mongoConverter, CacheableRepositoryHelper cacheableRepositoryHelper) {
        this.mongoOperations = mongoOperations;
        this.mongoConverter = mongoConverter;
        this.cacheableRepositoryHelper = cacheableRepositoryHelper;
        this.genericDomain = (Class<T>) GenericTypeResolver.resolveTypeArgument(getClass(), BaseBizbrainzRepositoryImpl.class);
    }
                    ...
                    return mongoOperations.updateFirst(query, updateObj, resource.getClass())
                            .flatMap(obj -> {
                                if (obj.getMatchedCount() == 0) {
                                    return Mono.error(new BizbrainzException(BizbrainzError.NO_RESOURCE_FOUND, resource.getClass().getSimpleName().toLowerCase(), id));
                                }
    ...
    public Mono<UpdateResult> updateByCriteria(List<Criteria> criteriaList, Update updateObj) {
        if (criteriaList == null) {
            return Mono.error(new BizbrainzException(BizbrainzError.INVALID_PARAMETER, "criteriaList"));
        }
