 ...
 * <p>
 * This was done because on authorization, the user needs to be stored in bizBrainz domain.
        ...
        if (!commonConfig.getOauthAllowedDomains().isEmpty()) {
            ...
            if (principal.getPrincipal() instanceof DefaultOidcUser) {
                ...
                if (!commonConfig.getOauthAllowedDomains().contains(domain)) {
                    return Mono.error(new BizbrainzException(BizbrainzError.UNAUTHORIZED_DOMAIN, domain));
                }
            }
        }
