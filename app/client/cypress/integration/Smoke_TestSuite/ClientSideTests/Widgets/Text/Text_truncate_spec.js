...
describe("Text Widget Truncate Functionality", function() {
  ...
  it("Validate long text is not truncating in default", function() {
    cy.get(
      `.bizBrainz_widget_${dsl.dsl.children[0].widgetId} .t--draggable-textwidget`,
    ).click({
      force: true,
    });
    ...
    cy.get(
      `.bizBrainz_widget_${dsl.dsl.children[0].widgetId} .t--widget-textwidget-truncate`,
    ).should("not.exist");
  });
  ...
  it("Add Long Text to large text box and validate", function() {
    cy.get(
      `.bizBrainz_widget_${dsl.dsl.children[1].widgetId} .t--draggable-textwidget`,
    ).click({
      force: true,
    });
    cy.wait(200);
    ...
    cy.get(
      `.bizBrainz_widget_${dsl.dsl.children[1].widgetId} .t--widget-textwidget-truncate`,
    ).should("not.exist");
  });
