...
describe("PgAdmin Clone App", function() {
  ...
  it("Create queries", function() {
    ...
    cy.get(".CodeMirror textarea")
      ...
      .focus()
      .type(
        'CREATE TABLE {{schema_select.selectedOptionValue}}.{{nt_name.text.replaceAll(" ","_")}}({{bizBrainz.store.nt_col.map((c)=>c.name.replaceAll(" ","_") + " " + c.dtype + (c.nnull ? " NOT NULL " :  "") + (c.pkey ? " PRIMARY KEY " : "")).join(" , ")}})',
        {
          force: true,
          parseSpecialCharSequences: false,
        },
