...
function setup_backend_heap_arg() {
    if [[ ! -z ${maximum_heap} ]]; then
      export BIZBRAINZ_JAVA_HEAP_ARG="-Xmx${maximum_heap}m"
    fi
}

init_env_file() {
  CONF_PATH="/bizBrainz-stacks/configuration"
  ENV_PATH="$CONF_PATH/docker.env"
  TEMPLATES_PATH="/opt/bizBrainz/templates"

  # Build an env file with current env variables. We single-quote the values, as well as escaping any single-quote characters.
  printenv | grep -E '^BIZBRAINZ_|^MONGO_' | sed "s/'/'\"'\"'/; s/=/='/; s/$/'/" > "$TEMPLATES_PATH/pre-define.env"
  ...
  if ! [[ -e "$ENV_PATH" ]]; then
    ...
    mkdir -p "$CONF_PATH"
    local default_bizBrainz_mongodb_user="bizBrainz"
    local generated_bizBrainz_mongodb_password=$(
      tr -dc A-Za-z0-9 </dev/urandom | head -c 13
      echo ""
    )
    local generated_bizBrainz_encryption_password=$(
      tr -dc A-Za-z0-9 </dev/urandom | head -c 13
      echo ""
    )
    local generated_bizBrainz_encription_salt=$(
      tr -dc A-Za-z0-9 </dev/urandom | head -c 13
      echo ""
    )
    local generated_bizBrainz_supervisor_password=$(
      tr -dc A-Za-z0-9 </dev/urandom | head -c 13
      echo ''
    )
    bash "$TEMPLATES_PATH/docker.env.sh" "$default_bizBrainz_mongodb_user" "$generated_bizBrainz_mongodb_password" "$generated_bizBrainz_encryption_password" "$generated_bizBrainz_encription_salt" "$generated_bizBrainz_supervisor_password" > "$ENV_PATH"
  fi
...
check_mongodb_uri() {
  echo "Checking BIZBRAINZ_MONGODB_URI"
  isUriLocal=1
  if [[ $BIZBRAINZ_MONGODB_URI == *"localhost"* || $BIZBRAINZ_MONGODB_URI == *"127.0.0.1"* ]]; then
...
init_replica_set() {
  ...
  if [[ $shouldPerformInitdb -gt 0 && $isUriLocal -eq 0 ]]; then
    ...
    bash "/opt/bizBrainz/templates/mongo-init.js.sh" "$BIZBRAINZ_MONGODB_USER" "$BIZBRAINZ_MONGODB_PASSWORD" > "/bizBrainz-stacks/configuration/mongo-init.js"
    mongo "127.0.0.1/bizBrainz" /bizBrainz-stacks/configuration/mongo-init.js
    ...
    sleep 10
    mongo "$BIZBRAINZ_MONGODB_URI" --eval 'rs.initiate()'
    mongod --dbpath "$MONGO_DB_PATH" --shutdown || true
  fi
  ...
  if [[ $isUriLocal -gt 0 ]]; then
    ...
    mongo_state="$(mongo --host "$BIZBRAINZ_MONGODB_URI" --quiet --eval "rs.status().ok")"
...
mount_letsencrypt_directory() {
  ...
  mkdir -p /bizBrainz-stacks/{letsencrypt,ssl}
  ln -s /bizBrainz-stacks/letsencrypt /etc/letsencrypt
}
...
configure_supervisord() {
  SUPERVISORD_CONF_PATH="/opt/bizBrainz/templates/supervisord"
  ...
  if [[ -z "${DYNO}" ]]; then
    ...
    if [[ $BIZBRAINZ_REDIS_URL == *"localhost"* || $BIZBRAINZ_REDIS_URL == *"127.0.0.1"* ]]; then
    ...
    if ! [[ -e "/bizBrainz-stacks/ssl/fullchain.pem" ]] || ! [[ -e "/bizBrainz-stacks/ssl/privkey.pem" ]]; then
...
# https://github.com/bizBrainzorg/bizBrainz/issues/11773
...
if ! [[ -e "$CREDENTIAL_PATH" ]]; then
  ...
  printf "$BIZBRAINZ_SUPERVISOR_USER:$(openssl passwd -apr1 $BIZBRAINZ_SUPERVISOR_PASSWORD)" > "$CREDENTIAL_PATH"
fi
# Ensure the restore path exists in the container, so an archive can be copied to it, if need be.
mkdir -p /bizBrainz-stacks/data/{backup,restore}

# Create sub-directory to store services log in the container mounting folder
mkdir -p /bizBrainz-stacks/logs/{backend,cron,editor,rts,mongodb,redis}
