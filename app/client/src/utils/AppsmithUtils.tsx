...
import { createMessage, ERROR_500 } from "@bizBrainz/constants/messages";
import { JSCollectionData } from "reducers/entityReducers/jsActionsReducer";
import { osName } from "react-device-detect";

export const initializeAnalyticsAndTrackers = () => {
  const bizBrainzConfigs = getBizbrainzConfigs();

  try {
    if (bizBrainzConfigs.sentry.enabled && !window.Sentry) {
      window.Sentry = Sentry;
      Sentry.init({
        ...bizBrainzConfigs.sentry,
  ...
  try {
    if (bizBrainzConfigs.smartLook.enabled && !(window as any).smartlook) {
      const { id } = bizBrainzConfigs.smartLook;
      AnalyticsUtil.initializeSmartLook(id);
    }

    if (bizBrainzConfigs.segment.enabled && !(window as any).analytics) {
      if (bizBrainzConfigs.segment.apiKey) {
        // This value is only enabled for Bizbrainz's cloud hosted version. It is not set in self-hosted environments
        AnalyticsUtil.initializeSegment(bizBrainzConfigs.segment.apiKey);
      } else if (bizBrainzConfigs.segment.ceKey) {
        // This value is set in self-hosted environments. But if the analytics are disabled, it's never used.
        AnalyticsUtil.initializeSegment(bizBrainzConfigs.segment.ceKey);
      }
    }
  } catch (e) {
