...
public class EncryptionHandler {
     ...
     * or are custom Bizbrainz types (and can hence have fields annotated for encryption within them),
     * or are parameterized collections of custom Bizbrainz types,
     * or are parameterized maps with custom Bizbrainz type values (keys are not scanned for encrypted fields)
     *
    ...
    List<CandidateField> findCandidateFieldsForType(@NonNull Object source) {
        ...
        // If it is not known, scan each field for annotation or Bizbrainz type
        ...
        synchronized (sourceClass) {
            ReflectionUtils.doWithFields(sourceClass, field -> {
                if (field.getAnnotation(Encrypted.class) != null) {
                ...
                } else if (BizbrainzDomain.class.isAssignableFrom(field.getType())) {
                    ...
                    if (fieldValue == null) {
                        if (this.encryptedFieldsMap.containsKey(field.getType())) {
                            ...
                            // then this is an bizBrainz field with known annotations
                            candidateField = new CandidateField(field, CandidateField.Type.BIZBRAINZ_FIELD_KNOWN);
                        } else {
                    ...
                    // If this is a collection, check if the Type parameter is an BizbrainzDomain
                    Type[] typeArguments;
                    ...
                    if(subFieldType != null) {
                        if (this.encryptedFieldsMap.containsKey(subFieldType)) {
                            // This is a known type, it should necessarily be of BizbrainzDomain type
                            assert BizbrainzDomain.class.isAssignableFrom(subFieldType);
                            ...
                            if (!existingSubTypeCandidates.isEmpty()) {
                                finalCandidateFields.add(new CandidateField(field, CandidateField.Type.BIZBRAINZ_COLLECTION_KNOWN));
                            }
                        } else if (BizbrainzDomain.class.isAssignableFrom(subFieldType)) {
                            ...
                            if (collection == null || collection.isEmpty()) {
                                finalCandidateFields.add(new CandidateField(field, CandidateField.Type.BIZBRAINZ_COLLECTION_UNKNOWN));
                            } else {
                                for (final Object o : collection) {
                                    ...
                                    if (o.getClass().getCanonicalName().equals(subFieldType.getTypeName())) {
                                        ...
                                        finalCandidateFields.add(new CandidateField(field, CandidateField.Type.BIZBRAINZ_COLLECTION_POLYMORPHIC));
                                    }
                                    break;
                                }
                            }
                    ...
                    if (this.encryptedFieldsMap.containsKey(subFieldType)) {
                        ...
                        final List<CandidateField> existingSubTypeCandidates = this.encryptedFieldsMap.get(subFieldType);
                        if (!existingSubTypeCandidates.isEmpty()) {
                            finalCandidateFields.add(new CandidateField(field, CandidateField.Type.BIZBRAINZ_MAP_KNOWN));
                        }
                    } else if (BizbrainzDomain.class.isAssignableFrom(subFieldType)) {
                        // If the type is not known, then this is either not parsed yet, or has polymorphic implementations
                        ...
                        if (map == null || map.isEmpty()) {
                            finalCandidateFields.add(new CandidateField(field, CandidateField.Type.BIZBRAINZ_MAP_UNKNOWN));
                        } else {
                            for (Map.Entry<?, ?> entry : map.entrySet()) {
                                ...
                                if (value.getClass().getCanonicalName().equals(subFieldType.getTypeName())) {
                                    ...
                                    finalCandidateFields.add(new CandidateField(field, CandidateField.Type.BIZBRAINZ_MAP_POLYMORPHIC));
                                }
                                break;
                            }
                        }
                    ...
                    BizbrainzDomain.class.isAssignableFrom(field.getType()) ||
            ...
            if (fieldValue != null) {
                if (CandidateField.Type.ANNOTATED_FIELD.equals(candidateField.getType())) {
                    ...
                    ReflectionUtils.setField(field, source, transformedValue);
                        ...
                        CandidateField.Type.BIZBRAINZ_FIELD_KNOWN,
                        CandidateField.Type.BIZBRAINZ_FIELD_UNKNOWN,
                        CandidateField.Type.BIZBRAINZ_FIELD_POLYMORPHIC)
                        .contains(candidateField.getType())) {
                    // or go into field type if it is not (this is an bizBrainz field)
                    ...
                    if (Set.of(
                            CandidateField.Type.BIZBRAINZ_COLLECTION_KNOWN,
                            CandidateField.Type.BIZBRAINZ_COLLECTION_UNKNOWN,
                            CandidateField.Type.BIZBRAINZ_COLLECTION_POLYMORPHIC)
                            .contains(candidateField.getType())) {
                        // This is a collection which will necessarily have elements of BizbrainzDomain type
                        ...
                        if (!subTypeHasEncrypted &&
                            ...
                            CandidateField.Type.BIZBRAINZ_MAP_KNOWN,
                            CandidateField.Type.BIZBRAINZ_MAP_UNKNOWN,
                            CandidateField.Type.BIZBRAINZ_MAP_POLYMORPHIC)
                            .contains(candidateField.getType())) {
                        // This is a map that will necessarily have element values of BizbrainzDomain type
                        boolean subTypeHasEncrypted = false;
