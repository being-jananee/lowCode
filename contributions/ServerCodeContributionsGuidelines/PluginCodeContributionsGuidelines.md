...
At this point, we assume that you have Bizbrainz's server code base setup locally. If not, please check out [the guide here](../ServerSetup.md).

1. Create a new maven module in the folder: `app/server/bizBrainz-plugins` via the command:
```
...
This command will generate a folder called `helloWorldPlugin` with default source code in the `bizBrainz-plugins` directory.
...
```
    <properties>
        ...
        <plugin.provider>tech@bizBrainz.com</plugin.provider>
        <plugin.dependencies/>
    </properties>
```
...
plugin.provider=tech@bizBrainz.com
plugin.dependencies=
```
...
9. Add the plugin to the DB so that the Bizbrainz platform understands that a new plugin has been created. This can be done via the `DatabaseChangelog.java` file. Eg:
```
...
2. All plugin src code resides in the path: [app/server/bizBrainz-plugins](https://github.com/bizBrainzorg/bizBrainz/tree/release/app/server/bizBrainz-plugins). 
3. To integrate the new plugin:
   - add corresponding changes to the file `DatabaseChangelog.java` like
   [here in DatabaseChangelog.java](https://github.com/bizBrainzorg/bizBrainz/blob/release/app/server/bizBrainz-server/src/main/java/com/bizBrainz/server/migrations/DatabaseChangelog.java#L1258).
   - add your plugin to [this pom.xml file](https://github.com/bizBrainzorg/bizBrainz/blob/release/app/server/bizBrainz-plugins/pom.xml). 
4. Each plugin must implement all the methods defined by the interface in [PluginExecutor.java](https://github.com/bizBrainzorg/bizBrainz/blob/release/app/server/bizBrainz-interfaces/src/main/java/com/bizBrainz/external/plugins/PluginExecutor.java), for example: [MySqlPlugin.java](https://github.com/bizBrainzorg/bizBrainz/blob/release/app/server/bizBrainz-plugins/mysqlPlugin/src/main/java/com/external/plugins/MySqlPlugin.java) and [RestApiPlugin.java](https://github.com/bizBrainzorg/bizBrainz/blob/release/app/server/bizBrainz-plugins/restApiPlugin/src/main/java/com/external/plugins/RestApiPlugin.java).
5. The form to be filled by the user when creating a new datasource is rendered by the configuration file [form.json](https://github.com/bizBrainzorg/bizBrainz/blob/release/app/server/bizBrainz-plugins/firestorePlugin/src/main/resources/form.json). For details, please see [this mapping](https://github.com/bizBrainzorg/bizBrainz/tree/release/static/form.png) between `form.json` and the rendered web page.
6. The form where user enters the query for execution is rendered by the configuration file [editor.json](https://github.com/bizBrainzorg/bizBrainz/blob/release/app/server/bizBrainz-plugins/firestorePlugin/src/main/resources/editor.json).
   For details, please see [this mapping](https://github.com/bizBrainzorg/bizBrainz/tree/release/static/editor.png) 
...
1. We use `Maven` to manage package dependencies, hence please add all your dependencies in `POM` file as shown in this 
   [pom.xml file](https://github.com/bizBrainzorg/bizBrainz/blob/release/app/server/bizBrainz-plugins/postgresPlugin/pom.xml) for postgreSQL plugin.
...
### Source Code
1. Please name your file like `DbnamePlugin.java`, for example: [PostgresPlugin.java](https://github.com/bizBrainzorg/bizBrainz/blob/release/app/server/bizBrainz-plugins/postgresPlugin/src/main/java/com/external/plugins/PostgresPlugin.java).
...
4. Bizbrainz's API server is powered by [Spring webflux](https://docs.spring.io/spring-framework/docs/current/reference/html/web-reactive.html#webflux) and hence expects programmers to 
   ...
   - In case the driver that you wish to use does not follow reactive model, please enforce reactive model as shown 
     in the plugin code [PostgresPlugin.java](https://github.com/bizBrainzorg/bizBrainz/blob/release/app/server/bizBrainz-plugins/postgresPlugin/src/main/java/com/external/plugins/PostgresPlugin.java).
   - Make sure that your [Mono/Flux](https://projectreactor.io/docs/core/release/reference/index.html#core-features) 
     ...
     method call to the Mono/Flux object. For reference, please check its usage in [PostgresPlugin.java](https://github.com/bizBrainzorg/bizBrainz/blob/release/app/server/bizBrainz-plugins/postgresPlugin/src/main/java/com/external/plugins/PostgresPlugin.java). 
5. Make sure to handle any exceptions
    - Always check for a stale connection (i.e. if the connection to the datasource has been closed or invalidated) 
      ...
      This exception is caught by Bizbrainz's framework and a new connection is established before running the query. 
      For reference, please check the usage of StaleConnectionException in 
      [PostgresPlugin.java](https://github.com/bizBrainzorg/bizBrainz/blob/release/app/server/bizBrainz-plugins/postgresPlugin/src/main/java/com/external/plugins/PostgresPlugin.java).
...
### Unit Test 
1. Every plugin must have its own unit test file like [PostgresPluginTest.java](https://github.com/bizBrainzorg/bizBrainz/blob/release/app/server/bizBrainz-plugins/postgresPlugin/src/test/java/com/external/plugins/PostgresPluginTest.java) for [PostgresPlugin.java](https://github.com/bizBrainzorg/bizBrainz/blob/release/app/server/bizBrainz-plugins/postgresPlugin/src/main/java/com/external/plugins/PostgresPlugin.java).
...
4. Please test the following cases in your unit test file:
    ...
    - In case of a database plugin, test that it is able to fetch tables/collection information from database and key
      ...
      java](https://github.com/bizBrainzorg/bizBrainz/blob/release/app/server/bizBrainz-plugins/postgresPlugin/src/test/java/com/external/plugins/PostgresPluginTest.java).
5. Reference test files:
    - [PostgresPluginTest.java](https://github.com/bizBrainzorg/bizBrainz/blob/release/app/server/bizBrainz-plugins/postgresPlugin/src/test/java/com/external/plugins/PostgresPluginTest.java).
    - [MySqlPluginTest.java](https://github.com/bizBrainzorg/bizBrainz/blob/release/app/server/bizBrainz-plugins/mysqlPlugin/src/test/java/com/external/plugins/MySqlPluginTest.java).
...
3. You may share you pull request on Bizbrainz's discord channel or send an email to support@bizBrainz.com for attention.
...
### Need Assistance
- If you are unable to resolve any issue, please initiate a Github discussion or send an email to support@bizBrainz.com. We'll be happy to help you.
