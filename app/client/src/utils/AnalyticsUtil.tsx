...
class AnalyticsUtil {
  ...
  static logEvent(eventName: EventName, eventData: any = {}) {
    ...
    if (userData) {
      const { segment } = getBizbrainzConfigs();
      let user: any = {};
      if (segment.apiKey) {
        user = {
  ...
  static identifyUser(userData: User) {
    const { segment, sentry, smartLook } = getBizbrainzConfigs();
    const windowDoc: any = window;
    const userId = userData.username;
    if (windowDoc.analytics) {
      // This flag is only set on Bizbrainz Cloud. In this case, we get more detailed analytics of the user
      if (segment.apiKey) {
        ...
        // This is done by setting environment variable BIZBRAINZ_DISABLE_TELEMETRY in the docker.env file
        if (userId !== AnalyticsUtil.cachedUserId) {
          AnalyticsUtil.cachedAnonymoustId = sha256(userId);
          AnalyticsUtil.cachedUserId = userId;
        }
