...
sudo echo "127.0.0.1	dev.bizBrainz.com" | sudo tee -a /etc/hosts
serve -s build -p 3000 &

# Substitute all the env variables in nginx
vars_to_substitute=$(printf '\$%s,' $(env | grep -o "^BIZBRAINZ_[A-Z0-9_]\+" | xargs))
cat ./docker/templates/nginx-app.conf.template | sed -e "s|__BIZBRAINZ_CLIENT_PROXY_PASS__|http://localhost:3000|g" | sed -e "s|__BIZBRAINZ_SERVER_PROXY_PASS__|http://localhost:8080|g" | envsubst ${vars_to_substitute} | sed -e 's|\${\(BIZBRAINZ_[A-Z0-9_]*\)}||g' > ./docker/nginx.conf
cat ./docker/templates/nginx-root.conf.template | envsubst ${vars_to_substitute} | sed -e 's|\${\(BIZBRAINZ_[A-Z0-9_]*\)}||g' > ./docker/nginx-root.conf

# Create the SSL files for Nginx. Required for service workers to work properly.
touch ./docker/dev.bizBrainz.com.pem ./docker/dev.bizBrainz.com-key.pem
echo "$BIZBRAINZ_SSL_CERTIFICATE" > ./docker/dev.bizBrainz.com.pem
echo "$BIZBRAINZ_SSL_KEY" > ./docker/dev.bizBrainz.com-key.pem

echo "Going to run the nginx server"
sudo docker pull nginx:latest
sudo docker pull bizBrainz/test-event-driver:latest
...
sudo docker run --network host --name wildcard-nginx -d -p 80:80 -p 443:443 \
    ...
    -v `pwd`/docker/dev.bizBrainz.com.pem:/etc/certificate/dev.bizBrainz.com.pem \
    -v `pwd`/docker/dev.bizBrainz.com-key.pem:/etc/certificate/dev.bizBrainz.com-key.pem \
    nginx:latest &
...
sudo docker run --name test-event-driver -d -p 2222:22 -p 5001:5001 -p 3306:3306 \
  ...
  -v ~/git-server/repos:/git-server/repos  bizBrainz/test-event-driver:latest
...
status_code=$(curl -o /dev/null -s -w "%{http_code}\n" https://dev.bizBrainz.com/api/v1/users)
