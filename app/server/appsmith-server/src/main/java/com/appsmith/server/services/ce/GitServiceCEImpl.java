...
import com.bizBrainz.server.constants.GitDefaultCommitMessage;
...
import com.bizBrainz.server.helpers.CollectionUtils;
import com.bizBrainz.server.helpers.GitCloudServicesUtils;
import com.bizBrainz.server.helpers.GitDeployKeyGenerator;
import com.bizBrainz.server.helpers.GitFileUtils;
import com.bizBrainz.server.helpers.GitUtils;
import com.bizBrainz.server.helpers.ResponseUtils;
import com.bizBrainz.server.migrations.JsonSchemaVersions;
...
import static com.bizBrainz.external.constants.GitConstants.CONFLICTED_SUCCESS_MESSAGE;
import static com.bizBrainz.external.constants.GitConstants.DEFAULT_COMMIT_MESSAGE;
import static com.bizBrainz.external.constants.GitConstants.EMPTY_COMMIT_ERROR_MESSAGE;
import static com.bizBrainz.external.constants.GitConstants.GIT_CONFIG_ERROR;
import static com.bizBrainz.external.constants.GitConstants.GIT_PROFILE_ERROR;
import static com.bizBrainz.external.constants.GitConstants.MERGE_CONFLICT_BRANCH_NAME;
import static com.bizBrainz.server.acl.AclPermission.MANAGE_ACTIONS;
...
import static com.bizBrainz.server.constants.CommentConstants.BIZBRAINZ_BOT_USERNAME;
import static com.bizBrainz.server.constants.FieldName.DEFAULT;
import static com.bizBrainz.server.helpers.DefaultResourcesUtils.createDefaultIdsOrUpdateWithGivenResourceIds;
...
public class GitServiceCEImpl implements GitServiceCE {
    ...
    public Mono<Application> updateGitMetadata(String applicationId, GitApplicationMetadata gitApplicationMetadata) {
        ...
        if (Optional.ofNullable(gitApplicationMetadata).isEmpty()) {
            return Mono.error(new BizbrainzException(BizbrainzError.INVALID_PARAMETER, "Git metadata values cannot be null"));
        }
    ...
    public Mono<Map<String, GitProfile>> updateOrCreateGitProfileForCurrentUser(GitProfile gitProfile, String defaultApplicationId) {
        ...
        if ((DEFAULT.equals(defaultApplicationId) || Boolean.FALSE.equals(gitProfile.getUseGlobalProfile()))
            ...
            return Mono.error(new BizbrainzException(BizbrainzError.INVALID_PARAMETER, "Author Name"));
            ...
            return Mono.error(new BizbrainzException(BizbrainzError.INVALID_PARAMETER, "Author Email"));
        } else if (StringUtils.isEmptyOrNull(defaultApplicationId)) {
            return Mono.error(new BizbrainzException(BizbrainzError.INVALID_PARAMETER, FieldName.APPLICATION_ID));
        }
                ...
                .flatMap(user -> userDataService.getForUser(user.getId())
                        .flatMap(userData -> {
                            ...
                            if (savedProfile == null || !savedProfile.equals(gitProfile) || defaultGitProfile == null) {
                                ...
                                // Assign bizBrainz user profile as a fallback git profile
                        ...
                        .switchIfEmpty(Mono.defer(() -> {
                                    // If profiles are empty use Bizbrainz's user profile as git default profile
    ...
    public Mono<GitProfile> getDefaultGitProfileOrCreateIfEmpty() {
        // Get default git profile if the default is empty then use Bizbrainz profile as a fallback value
        return getGitProfileForUser(DEFAULT)
                .flatMap(gitProfile -> {
                    if (StringUtils.isEmptyOrNull(gitProfile.getAuthorName()) || StringUtils.isEmptyOrNull(gitProfile.getAuthorEmail())) {
                        return updateGitProfileWithBizbrainzProfile(DEFAULT);
                    }
                    gitProfile.setUseGlobalProfile(null);
    ...
    private Mono<GitProfile> updateGitProfileWithBizbrainzProfile(String key) {
        return sessionUserService.getCurrentUser()
                .flatMap(user -> userService.findByEmail(user.getEmail()))
                .flatMap(currentUser -> {
    ...
    public Mono<String> commitApplication(GitCommitDTO commitDTO, String defaultApplicationId, String branchName, boolean doAmend) {
        ...
        if (StringUtils.isEmptyOrNull(branchName)) {
            throw new BizbrainzException(BizbrainzError.INVALID_PARAMETER, FieldName.BRANCH_NAME);
        }
        ...
        Mono<String> commitMono = this.getApplicationById(defaultApplicationId)
                .flatMap(defaultApplication -> {
                    ...
                    if (Optional.ofNullable(defaultGitMetadata).isEmpty()) {
                        return Mono.error(new BizbrainzException(BizbrainzError.INVALID_GIT_CONFIGURATION, GIT_CONFIG_ERROR));
                    }
                    ...
                    return applicationMono
                            ...
                            .flatMap(isRepoLimitReached -> {
                                ...
                                throw new BizbrainzException(BizbrainzError.GIT_APPLICATION_LIMIT_ERROR);
                            });
                })
                ...
                .flatMap(branchedApplication -> {
                    ...
                    if (!errorEntity.isEmpty()) {
                        return Mono.error(new BizbrainzException(BizbrainzError.INVALID_GIT_CONFIGURATION, "Unable to find " + errorEntity));
                    }
                    return Mono.zip(
                ...
                .onErrorResume(e -> {
                    ...
                    if (e instanceof RepositoryNotFoundException) {
                        return Mono.error(new BizbrainzException(BizbrainzError.REPOSITORY_NOT_FOUND, defaultApplicationId));
                    } else if (e instanceof BizbrainzException) {
                        return Mono.error(e);
                    }
                    return Mono.error(new BizbrainzException(BizbrainzError.GIT_FILE_SYSTEM_ERROR, e.getMessage()));
                })
                .flatMap(tuple -> {
                    ...
                    if (authorProfile == null || StringUtils.isEmptyOrNull(authorProfile.getAuthorName())) {
                        ...
                        return addAnalyticsForGitOperation(
                                ...
                                childApplication,
                                BizbrainzError.INVALID_GIT_CONFIGURATION.getErrorType(),
                                BizbrainzError.INVALID_GIT_CONFIGURATION.getMessage(errorMessage),
                                childApplication.getGitApplicationMetadata().getIsRepoPrivate()
                        )
                                .flatMap(user -> Mono.error(
                                        new BizbrainzException(BizbrainzError.INVALID_GIT_CONFIGURATION, errorMessage))
                                );
                    }
                    ...
                    Mono<String> gitCommitMono = gitExecutor
                            ...
                            .onErrorResume(error -> {
                                ...
                                return addAnalyticsForGitOperation(
                                        ...
                                        .then(Mono.error(new BizbrainzException(BizbrainzError.GIT_ACTION_FAILED, "commit", error.getMessage())));
                            });
        ...
        Mono<List<GitLogDTO>> commitHistoryMono = applicationService
                ...
                .flatMap(application -> {
                    ...
                    if (gitData == null || StringUtils.isEmptyOrNull(application.getGitApplicationMetadata().getBranchName())) {
                        return Mono.error(new BizbrainzException(
                                BizbrainzError.INVALID_GIT_CONFIGURATION,
                                GIT_CONFIG_ERROR
                        ));
                    }
                    ...
                    return Mono.zip(
                            gitExecutor.checkoutToBranch(baseRepoSuffix, gitData.getBranchName())
                                    .onErrorResume(e -> Mono.error(new BizbrainzException(BizbrainzError.GIT_ACTION_FAILED, "checkout", e.getMessage()))),
                ...
                .flatMap(tuple -> {
                    ...
                    return gitExecutor.getCommitHistory(baseRepoSuffix)
                            .onErrorResume(e -> Mono.error(new BizbrainzException(BizbrainzError.GIT_ACTION_FAILED, "log", e.getMessage())));
                });
    ...
    /**
     * Connect the application from Bizbrainz to a git repo
    ...
    public Mono<Application> connectApplicationToGit(String defaultApplicationId, GitConnectDTO gitConnectDTO, String originHeader) {
        ...
        if (StringUtils.isEmptyOrNull(gitConnectDTO.getRemoteUrl())) {
            return Mono.error(new BizbrainzException(BizbrainzError.INVALID_PARAMETER, "Remote Url"));
        }

        if (StringUtils.isEmptyOrNull(originHeader)) {
            return Mono.error(new BizbrainzException(BizbrainzError.INVALID_PARAMETER, FieldName.ORIGIN));
        }

        Mono<UserData> currentUserMono = userDataService.getForCurrentUser()
                .filter(userData -> !CollectionUtils.isNullOrEmpty(userData.getGitProfiles()))
                .switchIfEmpty(Mono.error(new BizbrainzException(BizbrainzError.INVALID_GIT_CONFIGURATION, GIT_PROFILE_ERROR)));
                    ...
                    return isRepoLimitReached(application.getWorkspaceId(), true)
                            .flatMap(isRepoLimitReached -> {
                                ...
                                return addAnalyticsForGitOperation(
                                        ...
                                        application,
                                        BizbrainzError.GIT_APPLICATION_LIMIT_ERROR.getErrorType(),
                                        BizbrainzError.GIT_APPLICATION_LIMIT_ERROR.getMessage(),
                                        application.getGitApplicationMetadata().getIsRepoPrivate()
                                ).flatMap(ignore -> Mono.error(new BizbrainzException(BizbrainzError.GIT_APPLICATION_LIMIT_ERROR)));
                            });
                })
                .flatMap(application -> {
                    GitApplicationMetadata gitApplicationMetadata = application.getGitApplicationMetadata();
                    if (isInvalidDefaultApplicationGitMetadata(application.getGitApplicationMetadata())) {
                        return Mono.error(new BizbrainzException(BizbrainzError.INVALID_GIT_SSH_CONFIGURATION));
                    } else {
                                    ...
                                    return fileUtils.deleteLocalRepo(repoSuffix)
                                            ...
                                            .flatMap(app -> {
                                                ...
                                                if (error instanceof InvalidRemoteException) {
                                                    return Mono.error(new BizbrainzException(BizbrainzError.INVALID_GIT_CONFIGURATION, error.getMessage()));
                                                }
                                                if (error instanceof TimeoutException) {
                                                    return Mono.error(new BizbrainzException(BizbrainzError.GIT_EXECUTION_TIMEOUT));
                                                }
                                                if (error instanceof ClassCastException) {
                                                    ...
                                                    if (error.getMessage().contains("TransportHttp")) {
                                                        return Mono.error(new BizbrainzException(BizbrainzError.INVALID_GIT_SSH_URL));
                                                    }
                                                }
                                                return Mono.error(new BizbrainzException(BizbrainzError.GIT_GENERIC_ERROR, error.getMessage()));
                                            });
                                });
                        return Mono.zip(
                                ...
                                .flatMap(objects -> {
                                    ...
                                    if (!isEmpty) {
                                        return addAnalyticsForGitOperation(
                                                AnalyticsEvents.GIT_CONNECT.getEventName(),
                                                application,
                                                BizbrainzError.INVALID_GIT_REPO.getErrorType(),
                                                BizbrainzError.INVALID_GIT_REPO.getMessage(),
                                                application.getGitApplicationMetadata().getIsRepoPrivate()
                                        )
                                                .then(Mono.error(new BizbrainzException(BizbrainzError.INVALID_GIT_REPO)));
                                    } else {
                                        GitApplicationMetadata gitApplicationMetadata = application.getGitApplicationMetadata();
                    ...
                    try {
                        return Mono.zip(
                                        fileUtils.initializeReadme(
                                            ...
                                            return new BizbrainzException(
                                                    BizbrainzError.GIT_FILE_SYSTEM_ERROR,
                                                            ...
                                                            .flatMap(isDeleted -> {
                                                                if (error instanceof TransportException) {
                                                                    return addAnalyticsForGitOperation(
                                                                            ...
                                                                            .then(Mono.error(new BizbrainzException(
                                                                                    BizbrainzError.INVALID_GIT_SSH_CONFIGURATION, error.getMessage()))
                                                                            );
                                                                }
                                                                return Mono.error(new BizbrainzException(BizbrainzError.GIT_ACTION_FAILED, "push", error.getMessage()));
                                                            })
                                            );
                                })
                                .then(addAnalyticsForGitOperation(
                ...
                .flatMap(tuple -> {
                    ...
                    if (pushResult.contains("REJECTED")) {
                        ...
                        return addAnalyticsForGitOperation(
                                AnalyticsEvents.GIT_PUSH.getEventName(),
                                application,
                                BizbrainzError.GIT_UPSTREAM_CHANGES.getErrorType(),
                                BizbrainzError.GIT_UPSTREAM_CHANGES.getMessage(),
                                application.getGitApplicationMetadata().getIsRepoPrivate()
                        )
                                .flatMap(application1 -> Mono.error(new BizbrainzException(BizbrainzError.GIT_UPSTREAM_CHANGES)));
                    }
        ...
        Mono<Application> disconnectMono = getApplicationById(defaultApplicationId)
                .flatMap(defaultApplication -> {
                    if (Optional.ofNullable(defaultApplication.getGitApplicationMetadata()).isEmpty()
                        ...
                        return Mono.error(
                                new BizbrainzException(BizbrainzError.INVALID_GIT_CONFIGURATION, "Please reconfigure the application to connect to git repo")
                        );
                    }
    ...
    public Mono<Application> createBranch(String defaultApplicationId, GitBranchDTO branchDTO, String srcBranch) {
        ...
        if (StringUtils.isEmptyOrNull(srcBranch)
            ...
            return Mono.error(new BizbrainzException(BizbrainzError.INVALID_PARAMETER, FieldName.BRANCH_NAME));
        }
        ...
        Mono<Application> createBranchMono = applicationService
                ...
                .flatMap(tuple -> {
                    ...
                    return gitExecutor.checkoutToBranch(repoSuffix, srcBranch)
                            .onErrorResume(error -> Mono.error(new BizbrainzException(BizbrainzError.GIT_ACTION_FAILED, "checkout", "Unable to find " + srcBranch)))
                            .zipWhen(isCheckedOut -> gitExecutor.fetchRemote(repoSuffix, defaultGitAuth.getPublicKey(), defaultGitAuth.getPrivateKey(), false, srcBranch, true)
                                    .onErrorResume(error -> Mono.error(new BizbrainzException(BizbrainzError.GIT_ACTION_FAILED, "fetch", error))))
                            .flatMap(ignore -> gitExecutor.listBranches(repoSuffix, srcBranchGitData.getRemoteUrl(), defaultGitAuth.getPrivateKey(), defaultGitAuth.getPublicKey(), false)
                                    .flatMap(branchList -> {
                                        ...
                                        if (isDuplicateName) {
                                            return Mono.error(new BizbrainzException(
                                                    BizbrainzError.DUPLICATE_KEY_USER_ERROR,
                            ...
                            .onErrorResume(error -> Mono.error(new BizbrainzException(BizbrainzError.GIT_ACTION_FAILED, "branch", error.getMessage())));
                })
                .flatMap(tuple -> {
            ...
            return listBranchForApplication(defaultApplicationId, false, branchName)
                    .flatMap(gitBranchDTOList -> {
                        ...
                        if (branchMatchCount == 0) {
                            ...
                            return Mono.error(
                                    new BizbrainzException(BizbrainzError.GIT_ACTION_FAILED, "checkout", branchName + " already exists in local - " + branchName.replaceFirst("origin/", "")));
                        }
                    });
        }
        ...
        Mono<Application> checkoutRemoteBranchMono = getApplicationById(defaultApplicationId)
                .flatMap(application -> {
                    ...
                    return gitExecutor.fetchRemote(repoPath,
                            ...
                            .onErrorResume(error -> Mono.error(new BizbrainzException(BizbrainzError.GIT_ACTION_FAILED, "checkout branch", error.getMessage()))));
                })
                .flatMap(tuple -> {
                    /*
                    ...
                    return applicationService.save(srcApplication)
                            ...
                            // We need to handle the case specifically for default branch of Bizbrainz
                            ...
                            .onErrorResume(throwable -> {
                                ...
                                return Mono.error(new BizbrainzException(BizbrainzError.GIT_ACTION_FAILED, " --checkout", throwable.getMessage()));
                            });
                })
                .flatMap(tuple -> {
    ...
    Mono<Application> getApplicationById(String applicationId) {
        return applicationService.findById(applicationId, MANAGE_APPLICATIONS)
                .switchIfEmpty(Mono.error(new BizbrainzException(BizbrainzError.NO_RESOURCE_FOUND, FieldName.APPLICATION_ID, applicationId)));
    }
        ...
        Mono<GitPullDTO> pullMono = getApplicationById(defaultApplicationId)
                ...
                .flatMap(tuple -> {
                    ...
                    if (!CollectionUtils.isNullOrEmpty(status.getModified())) {
                        return Mono.error(new BizbrainzException(
                                BizbrainzError.GIT_ACTION_FAILED,
                                "pull",
        ...
        Mono<GitStatusDTO> statusMono = Mono.zip(
                ...
                .flatMap(tuple -> {
                    ...
                    try {
                        ...
                        return Mono.error(new BizbrainzException(BizbrainzError.GIT_ACTION_FAILED, "status", e.getMessage()));
                    }
                })
                .flatMap(tuple -> {
                    ...
                    try {
                        return gitExecutor.fetchRemote(tuple.getT1(), tuple.getT2().getPublicKey(), tuple.getT2().getPrivateKey(), true, branchName, false)
                                ...
                                .then(branchedStatusMono)
                                .onErrorResume(error -> Mono.error(new BizbrainzException(BizbrainzError.GIT_ACTION_FAILED, "status", error.getMessage())));
                    } catch (GitAPIException | IOException e) {
                        return Mono.error(new BizbrainzException(BizbrainzError.GIT_GENERIC_ERROR, e.getMessage()));
                    }
                });
    ...
    public Mono<MergeStatusDTO> mergeBranch(String defaultApplicationId, GitMergeDTO gitMergeDTO) {
        ...
        if (StringUtils.isEmptyOrNull(sourceBranch) || StringUtils.isEmptyOrNull(destinationBranch)) {
            ...
            return Mono.error(new BizbrainzException(BizbrainzError.UNSUPPORTED_OPERATION_FOR_REMOTE_BRANCH, sourceBranch));
        } else if (destinationBranch.startsWith("origin/")) {
            return Mono.error(new BizbrainzException(BizbrainzError.UNSUPPORTED_OPERATION_FOR_REMOTE_BRANCH, destinationBranch));
        }
                ...
                .flatMap(defaultApplication -> {
                    ...
                    Mono<Path> pathToFile = this.getStatus(defaultApplicationId, sourceBranch)
                            .flatMap(status -> {
                                if (!Integer.valueOf(0).equals(status.getBehindCount())) {
                                    throw Exceptions.propagate(new BizbrainzException(BizbrainzError.GIT_MERGE_FAILED_REMOTE_CHANGES, status.getBehindCount(), sourceBranch));
                                } else if (!CollectionUtils.isNullOrEmpty(status.getModified())) {
                                    throw Exceptions.propagate(new BizbrainzException(BizbrainzError.GIT_MERGE_FAILED_LOCAL_CHANGES, sourceBranch));
                                }
                                return this.getStatus(defaultApplicationId, destinationBranch)
                                        .map(status1 -> {
                                            if (!Integer.valueOf(0).equals(status.getBehindCount())) {
                                                throw Exceptions.propagate(new BizbrainzException(BizbrainzError.GIT_MERGE_FAILED_REMOTE_CHANGES, status.getBehindCount(), destinationBranch));
                                            } else if (!CollectionUtils.isNullOrEmpty(status.getModified())) {
                                                throw Exceptions.propagate(new BizbrainzException(BizbrainzError.GIT_MERGE_FAILED_LOCAL_CHANGES, destinationBranch));
                                            }
                                            return status1;
                                        });
                            })
                    ...
                    return Mono.zip(
                            ...
                            .onErrorResume(error -> {
                                ...
                                if (error instanceof BizbrainzException) {
                                    return Mono.error(error);
                                }
                                return Mono.error(new BizbrainzException(BizbrainzError.GIT_ACTION_FAILED, "status", error));
                            });
                })
                .flatMap(tuple -> {
                    Application defaultApplication = tuple.getT1();
                            ...
                            .onErrorResume(error -> addAnalyticsForGitOperation(
                                            ...
                                            .flatMap(application -> {
                                                if (error instanceof GitAPIException) {
                                                    return Mono.error(new BizbrainzException(BizbrainzError.GIT_MERGE_CONFLICTS, error.getMessage()));
                                                }
                                                return Mono.error(new BizbrainzException(BizbrainzError.GIT_ACTION_FAILED, "merge", error.getMessage()));
                                            })
                            );
                })
                    ...
                    return this.getStatus(defaultApplicationId, sourceBranch)
                            .flatMap(srcBranchStatus -> {
                                if (!Integer.valueOf(0).equals(srcBranchStatus.getBehindCount())) {
                                    return Mono.error(Exceptions.propagate(new BizbrainzException(BizbrainzError.GIT_MERGE_FAILED_REMOTE_CHANGES, srcBranchStatus.getBehindCount(), sourceBranch)));
                                } else if (!CollectionUtils.isNullOrEmpty(srcBranchStatus.getModified())) {
                                    return Mono.error(Exceptions.propagate(new BizbrainzException(BizbrainzError.GIT_MERGE_FAILED_LOCAL_CHANGES, sourceBranch)));
                                }
                                return this.getStatus(defaultApplicationId, destinationBranch)
                                        .map(destBranchStatus -> {
                                            if (!Integer.valueOf(0).equals(destBranchStatus.getBehindCount())) {
                                                throw Exceptions.propagate(new BizbrainzException(BizbrainzError.GIT_MERGE_FAILED_REMOTE_CHANGES, destBranchStatus.getBehindCount(), destinationBranch));
                            ...
                            .onErrorResume(error -> {
                                try {
                                    ...
                                    return Mono.error(new BizbrainzException(BizbrainzError.GIT_ACTION_FAILED, "reset --hard HEAD", e.getMessage()));
                                }
                            });
                });
        ...
        Mono<String> conflictedBranchMono = Mono.zip(
                ...
                .flatMap(tuple -> {
                    ...
                    try {
                        ...
                        return Mono.error(new BizbrainzException(BizbrainzError.GIT_ACTION_FAILED, "checkout", e.getMessage()));
                    }
                })
                .flatMap(tuple -> {
    ...
    public Mono<ApplicationImportDTO> importApplicationFromGit(String workspaceId, GitConnectDTO gitConnectDTO) {
        ...
        if (StringUtils.isEmptyOrNull(workspaceId)) {
            return Mono.error(new BizbrainzException(BizbrainzError.INVALID_PARAMETER, "Invalid workspace id"));
        }
        ...
        Mono<ApplicationImportDTO> importedApplicationMono = getSSHKeyForCurrentUser()
                ...
                .switchIfEmpty(Mono.error(new BizbrainzException(BizbrainzError.INVALID_GIT_CONFIGURATION,
                        "Unable to find git configuration for logged-in user. Please contact Bizbrainz team for support")))
                //Check the limit for number of private repo
                .flatMap(tuple -> {
                    // Check if the repo is public
                    ...
                    return isRepoLimitReached(workspaceId, true)
                            .flatMap(isRepoLimitReached -> {
                                ...
                                return addAnalyticsForGitOperation(
                                        ...
                                        BizbrainzError.GIT_APPLICATION_LIMIT_ERROR.getMessage(),
                                        false
                                ).flatMap(user -> Mono.error(new BizbrainzException(BizbrainzError.GIT_APPLICATION_LIMIT_ERROR)));
                            });
                })
                .flatMap(tuple -> {
                    GitAuth gitAuth = tuple.getT1();
                    ...
                    Mono<String> defaultBranchMono = gitExecutor.cloneApplication(
                        ...
                        return addAnalyticsForGitOperation(
                                ...
                                .flatMap(application1 -> {
                                    ...
                                    return Mono.error(new BizbrainzException(BizbrainzError.GIT_ACTION_FAILED, "clone", error));
                                });
                    });
                    ...
                    Mono<ApplicationJson> applicationJsonMono = fileUtils
                            ...
                            .onErrorResume(error -> {
                                ...
                                return deleteApplicationCreatedFromGitImport(application.getId(), application.getWorkspaceId(), gitApplicationMetadata.getRepoName())
                                        .flatMap(application1 -> Mono.error(new BizbrainzException(BizbrainzError.GIT_FILE_SYSTEM_ERROR, error.getMessage())));
                            });
                    ...
                    return Mono.zip(applicationJsonMono, datasourceMono, pluginMono)
                            .flatMap(data -> {
                                ...
                                if (Optional.ofNullable(applicationJson.getExportedApplication()).isEmpty()
                                    ...
                                    return deleteApplicationCreatedFromGitImport(application.getId(), application.getWorkspaceId(), gitApplicationMetadata.getRepoName())
                                            .then(Mono.error(new BizbrainzException(BizbrainzError.GIT_ACTION_FAILED, "import", "Cannot import app from an empty repo")));
                                }
                                ...
                                if (checkIsDatasourceNameConflict(datasourceList, applicationJson.getDatasourceList(), pluginList)) {
                                    return deleteApplicationCreatedFromGitImport(application.getId(), application.getWorkspaceId(), gitApplicationMetadata.getRepoName())
                                            .then(Mono.error(new BizbrainzException(BizbrainzError.GIT_ACTION_FAILED,
                                                    "import",
                                ...
                                return importExportApplicationService
                                        ...
                                        .onErrorResume(throwable ->
                                                deleteApplicationCreatedFromGitImport(application.getId(), application.getWorkspaceId(), gitApplicationMetadata.getRepoName())
                                                        .flatMap(application1 -> Mono.error(new BizbrainzException(BizbrainzError.GIT_FILE_SYSTEM_ERROR, throwable.getMessage())))
                                        );
                            });
                })
        ...
        Mono<Application> deleteBranchMono = getApplicationById(defaultApplicationId)
                .flatMap(application -> {
                    ...
                    if (branchName.equals(gitApplicationMetadata.getDefaultBranchName())) {
                        return Mono.error(new BizbrainzException(BizbrainzError.GIT_ACTION_FAILED, "delete branch", " Cannot delete default branch"));
                    }
                    return gitExecutor.deleteBranch(repoPath, branchName)
                            .onErrorResume(throwable -> {
                                ...
                                if (throwable instanceof CannotDeleteCurrentBranchException) {
                                    return Mono.error(new BizbrainzException(BizbrainzError.GIT_ACTION_FAILED, "delete branch", "Cannot delete current checked out branch"));
                                }
                                return Mono.error(new BizbrainzException(BizbrainzError.GIT_ACTION_FAILED, "delete branch", throwable.getMessage()));
                            })
                            .flatMap(isBranchDeleted -> {
                                if (Boolean.FALSE.equals(isBranchDeleted)) {
                                    return Mono.error(new BizbrainzException(BizbrainzError.GIT_ACTION_FAILED, " delete branch. Branch does not exists in the repo"));
                                }
    ...
    private Mono<String> commitAndPushWithDefaultCommit(Path repoSuffix,
        ...
        return gitExecutor.commitApplication(repoSuffix, DEFAULT_COMMIT_MESSAGE + reason.getReason(), BIZBRAINZ_BOT_USERNAME, emailConfig.getSupportEmailAddress(), true, false)
                .onErrorResume(error -> {
                    if (error instanceof EmptyCommitException) {
                        return Mono.just(EMPTY_COMMIT_ERROR_MESSAGE);
                    }
                    return Mono.error(new BizbrainzException(BizbrainzError.GIT_ACTION_FAILED, "commit", error.getMessage()));
                })
                .flatMap(commitMessage ->
                        gitExecutor.pushApplication(
                                ...
                                .map(pushResult -> {
                                    if (pushResult.contains("REJECTED")) {
                                        throw new BizbrainzException(BizbrainzError.GIT_UPSTREAM_CHANGES);
                                    }
                                    return pushResult;
                                })
                );
                    ...
                    return gitExecutor.cloneApplication(repoPath, gitApplicationMetadata.getRemoteUrl(), gitAuth.getPrivateKey(), gitAuth.getPublicKey())
                            ...
                            .flatMap(gitBranchDTOList -> {
                                List<String> branchList = gitBranchDTOList.stream()
                                        ...
                                        .collect(Collectors.toList());
                                // Remove the default branch of Bizbrainz
                        ...
                        Mono<MergeStatusDTO> pullStatusMono = gitExecutor.checkoutToBranch(repoSuffix, branchName)
                                ...
                                .onErrorResume(error -> {
                                    if (error.getMessage().contains("conflict")) {
                                        return Mono.error(new BizbrainzException(BizbrainzError.GIT_PULL_CONFLICTS, error.getMessage()));
                                    ...
                                    return Mono.error(new BizbrainzException(BizbrainzError.GIT_ACTION_FAILED, "pull", error.getMessage()));
                                })
                                .cache();
