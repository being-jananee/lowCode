...
public class MarketplaceServiceCEImpl implements MarketplaceServiceCE {
    ...
    public Mono<ProviderPaginatedDTO> getProviders(MultiValueMap<String, String> params) {
        ...
        if (uri == null) {
            return Mono.error(new BizbrainzException(BizbrainzError.MARKETPLACE_NOT_CONFIGURED));
        }
        ...
        return webClient
                ...
                .flatMap(stringBody -> {
                    ...
                    try {
                        ...
                        return Mono.error(new BizbrainzException(BizbrainzError.JSON_PROCESSING_ERROR, e));
                    }
                    return Mono.just(providersPaginated);
                })
                .timeout(Duration.ofMillis(timeoutInMillis))
                .doOnError(error -> Mono.error(new BizbrainzException(BizbrainzError.MARKETPLACE_TIMEOUT)));
    }
