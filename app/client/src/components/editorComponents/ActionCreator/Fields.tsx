...
import { ALERT_STYLE_OPTIONS } from "@bizBrainz/constants/messages";
...
const fieldConfigs: FieldConfigs = {
  [FieldType.ACTION_SELECTOR_FIELD]: {
    getter: (storedValue: string) => {
      ...
      let mainFuncSelectedValue = BizbrainzFunction.none;
      if (matches.length) {
        mainFuncSelectedValue = matches[0][1] || BizbrainzFunction.none;
      }
    ...
    setter: (option: TreeDropdownOption) => {
      ...
      switch (type) {
        case BizbrainzFunction.integration:
          value = `${value}.run`;
          break;
        case BizbrainzFunction.navigateTo:
          defaultParams = `'', {}, 'SAME_WINDOW'`;
          break;
        case BizbrainzFunction.jsFunction:
          defaultArgs = option.args ? option.args : [];
          break;
        case BizbrainzFunction.setInterval:
          defaultParams = "() => { \n\t // add code here \n}, 5000";
          break;
        case BizbrainzFunction.getGeolocation:
          defaultParams = "(location) => { \n\t // add code here \n  }";
          break;
        case BizbrainzFunction.resetWidget:
          defaultParams = `"",true`;
          break;
        default:
          break;
    ...
    case FieldType.WIDGET_NAME_FIELD:
      ...
      if (fieldType === FieldType.ACTION_SELECTOR_FIELD) {
        ...
        selectedLabelModifier = function(
          ...
          if (option.type === BizbrainzFunction.integration) {
            return (
              <HightlightedCode
        ...
        getDefaults = (value: string) => {
          return {
            [BizbrainzFunction.navigateTo]: `'${props.pageDropdownOptions[0].label}'`,
          }[value];
        };
      }
      if (
