...
import { isAction, isBizbrainzEntity, isTrueObject } from "./evaluationUtils";
import {
  ActionDescription,
  ActionTriggerType,
...
> = {
  ...
  storeValue: function(key: string, value: string, persist = true) {
    ...
    _.set(self, ["bizBrainz", "store", key], value);
    return {
  ...
  getGeoLocation: {
    qualifier: (entity) => isBizbrainzEntity(entity),
    path: "bizBrainz.geolocation.getCurrentPosition",
    func: () =>
      function(
  ...
  watchGeoLocation: {
    qualifier: (entity) => isBizbrainzEntity(entity),
    path: "bizBrainz.geolocation.watchPosition",
    func: () =>
      function(
  ...
  stopWatchGeoLocation: {
    qualifier: (entity) => isBizbrainzEntity(entity),
    path: "bizBrainz.geolocation.clearWatch",
    func: () =>
      function() {
        return {
