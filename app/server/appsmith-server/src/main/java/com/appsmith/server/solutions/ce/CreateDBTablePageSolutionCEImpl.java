...
import com.bizBrainz.external.models.DatasourceStructure.Column;
import com.bizBrainz.external.models.DatasourceStructure.PrimaryKey;
import com.bizBrainz.external.models.DatasourceStructure.Table;
...
import com.bizBrainz.server.dtos.CRUDPageResourceDTO;
import com.bizBrainz.server.dtos.CRUDPageResponseDTO;
...
public class CreateDBTablePageSolutionCEImpl implements CreateDBTablePageSolutionCE {
    ...
    public Mono<CRUDPageResponseDTO> createPageFromDBTable(String defaultPageId,
        ...
        if (pageResourceDTO.getTableName() == null || pageResourceDTO.getDatasourceId() == null) {
            return Mono.error(new BizbrainzException(BizbrainzError.INVALID_PARAMETER, "tableName and datasourceId"));
        ...
        Mono<Datasource> datasourceMono = datasourceService
                ...
                .switchIfEmpty(Mono.error(
                        new BizbrainzException(BizbrainzError.ACL_NO_RESOURCE_FOUND, FieldName.DATASOURCE, datasourceId))
                )
                .filter(Datasource::getIsValid)
                .switchIfEmpty(Mono.error(
                        new BizbrainzException(BizbrainzError.INVALID_DATASOURCE, FieldName.DATASOURCE, datasourceId))
                );
        ...
        return datasourceMono
                ...
                .flatMap(tuple -> {
                    ...
                    try {
                        BizbrainzBeanUtils.copyNestedNonNullProperties(fetchTemplateApplication(FILE_PATH), applicationJson);
                    } catch (IOException e) {
                    ...
                    if (pageList.isEmpty()) {
                        return Mono.error(new BizbrainzException(BizbrainzError.NO_RESOURCE_FOUND, TEMPLATE_APPLICATION_FILE));
                    }
                    ...
                    if (templatePage == null) {
                        return Mono.error(
                                new BizbrainzException(BizbrainzError.UNSUPPORTED_OPERATION, plugin.getName())
                        );
                    }
                    ...
                    if (layout == null) {
                        return Mono.error(new BizbrainzException(BizbrainzError.NO_RESOURCE_FOUND, FieldName.PAGE));
                    }
                    layout.setId(null);
                    ...
                    if (templateStructure != null && !CollectionUtils.isEmpty(templateStructure.getTables())) {
                        ...
                        if (table == null) {
                            return Mono.error(
                                    new BizbrainzException(BizbrainzError.NO_RESOURCE_FOUND, FieldName.DATASOURCE_STRUCTURE, datasource.getName())
                            );
                        }
            ...
            return newPageService.findByBranchNameAndDefaultPageId(branchName, defaultPageId, MANAGE_PAGES)
                    ...
                    .map(newPage -> {
                        ...
                        if (!CollectionUtils.isEmpty(layout.getWidgetNames()) && layout.getWidgetNames().size() > 1) {
                            throw new BizbrainzException(BizbrainzError.INVALID_CRUD_PAGE_REQUEST, "please use empty layout");
                        }
                        return newPage;
                    });
        }
                        ...
                        pluginSpecifiedTemplates.forEach(property -> {
                            if (property != null && property.getValue() instanceof String) {
                                if (Entity.S3_PLUGIN_PACKAGE_NAME.equals(templateAction.getPluginId()) && mappedColumns.containsKey(property.getValue().toString())) {
                                    ...
                                    // matcher as the bucket name can be test.bizBrainz etc
