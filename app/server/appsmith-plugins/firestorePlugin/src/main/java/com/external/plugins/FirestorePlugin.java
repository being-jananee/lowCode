...
public class FirestorePlugin extends BasePlugin {
    ...
    public static class FirestorePluginExecutor implements PluginExecutor<Firestore>, SmartSubstitutionInterface {
        ...
        private void insertFieldValues(Map<String, Object> mapBody,
                                       ...
                                       Method method,
                                       List<RequestParamDTO> requestParams) throws BizbrainzPluginException {
            ...
            if (!Method.UPDATE_DOCUMENT.equals(method)
                ...
                throw new BizbrainzPluginException(
                        ...
                        "Bizbrainz has found an unexpected query form property - 'Delete Key Value Pair Path'. Please " +
            ...
            if (!isBlank(PluginUtils.getDataValueSafelyFromFormData(formData, DELETE_KEY_PATH, STRING_TYPE))) {
                ...
                try {
                    ...
                    throw new BizbrainzPluginException(
                            BizbrainzPluginError.PLUGIN_EXECUTE_ARGUMENT_ERROR,
                            "Bizbrainz failed to parse the query editor form field 'Delete Key Value Pair Path'. " +
                                    "Please check out Bizbrainz's documentation to find the correct syntax."
                    );
                }
            ...
            if (isGetOrDeleteMethod(method)
                ...
                throw new BizbrainzPluginException(
                        BizbrainzPluginError.PLUGIN_ERROR,
                        "Bizbrainz has found an unexpected query form property - 'Timestamp Value Path'. Please reach " +
            ...
            if (!isBlank(PluginUtils.getDataValueSafelyFromFormData(formData, TIMESTAMP_VALUE_PATH, STRING_TYPE))) {
                ...
                try {
                    ...
                    throw new BizbrainzPluginException(
                            BizbrainzPluginError.PLUGIN_EXECUTE_ARGUMENT_ERROR,
                            "Bizbrainz failed to parse the query editor form field 'Timestamp Value Path'. " +
                                    "Please check out Bizbrainz's documentation to find the correct syntax."
                    );
                }
        ...
        private Mono<ActionExecutionResult> methodGetCollection(CollectionReference query, Map<String, Object> formData,
            ...
            try {
                ...
                return Mono.error(new BizbrainzPluginException(BizbrainzPluginError.PLUGIN_JSON_PARSE_ERROR, orderByString, e));
            }
            ...
            if (PaginationField.NEXT.equals(paginationField)) {
                try {
                    ...
                    return Mono.error(new BizbrainzPluginException(BizbrainzPluginError.PLUGIN_JSON_PARSE_ERROR, startAfterJson, e));
                }
            }
            ...
            if (PaginationField.PREV.equals(paginationField)) {
                try {
                    ...
                    return Mono.error(new BizbrainzPluginException(BizbrainzPluginError.PLUGIN_JSON_PARSE_ERROR, endBeforeJson, e));
                }
            }
        ...
        private Object resultToMap(Object objResult) throws BizbrainzPluginException {
            return resultToMap(objResult, true);
        }

        private Object resultToMap(Object objResult, boolean isRoot) throws BizbrainzPluginException {
        ...
        public Mono<Firestore> datasourceCreate(DatasourceConfiguration datasourceConfiguration) {
            ...
            if (!CollectionUtils.isEmpty(errors)) {
                return Mono.error(new BizbrainzPluginException(BizbrainzPluginError.PLUGIN_DATASOURCE_ARGUMENT_ERROR,
            ...
            return Mono
                    .fromSupplier(() -> {
                        ...
                        try {
                            ...
                            throw Exceptions.propagate(new BizbrainzPluginException(
