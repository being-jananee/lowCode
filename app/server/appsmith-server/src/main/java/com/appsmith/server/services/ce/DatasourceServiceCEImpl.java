...
public class DatasourceServiceCEImpl extends BaseService<DatasourceRepository, Datasource, String> implements DatasourceServiceCE {
        ...
        return userMono
                .flatMap(user -> {
                    Mono<Workspace> workspaceMono = workspaceService.findById(datasource.getWorkspaceId(), WORKSPACE_MANAGE_DATASOURCES)
                            .switchIfEmpty(Mono.error(new BizbrainzException(BizbrainzError.NO_RESOURCE_FOUND, FieldName.WORKSPACE, datasource.getWorkspaceId())));
    ...
    public Mono<Datasource> getValidDatasourceFromActionMono(ActionDTO actionDTO, AclPermission aclPermission) {
        ...
        if (actionDTO.getDatasource() != null && actionDTO.getDatasource().getId() != null) {
            return this.findById(actionDTO.getDatasource().getId(), aclPermission)
                    .switchIfEmpty(Mono.error(new BizbrainzException(BizbrainzError.NO_RESOURCE_FOUND,
    ...
    public Mono<Datasource> update(String id, Datasource datasource, Boolean isServerRefreshedUpdate) {
        ...
        Mono<Datasource> datasourceMono = repository.findById(id)
                .switchIfEmpty(Mono.error(new BizbrainzException(BizbrainzError.NO_RESOURCE_FOUND, FieldName.DATASOURCE, id)));
    ...
    public Mono<Datasource> validateDatasource(Datasource datasource) {
        ...
        if (datasource.getPluginId() == null) {
            invalids.add(BizbrainzError.PLUGIN_ID_NOT_GIVEN.getMessage());
            return Mono.just(datasource);
        }

        if (datasource.getWorkspaceId() == null) {
            invalids.add(BizbrainzError.WORKSPACE_ID_NOT_GIVEN.getMessage());
            return Mono.just(datasource);
        }
        ...
        Mono<Workspace> checkPluginInstallationAndThenReturnWorkspaceMono = workspaceService
                ...
                .switchIfEmpty(Mono.defer(() -> {
                    invalids.add(BizbrainzError.PLUGIN_NOT_INSTALLED.getMessage(datasource.getPluginId()));
                    return Mono.just(new Workspace());
                }));

        if (datasource.getDatasourceConfiguration() == null) {
            invalids.add(BizbrainzError.NO_CONFIGURATION_FOUND_IN_DATASOURCE.getMessage());
        }

        final Mono<Plugin> pluginMono = pluginService.findById(datasource.getPluginId()).cache();
        Mono<PluginExecutor> pluginExecutorMono = pluginExecutorHelper.getPluginExecutor(pluginMono)
                .switchIfEmpty(Mono.error(new BizbrainzException(BizbrainzError.NO_RESOURCE_FOUND, FieldName.PLUGIN, datasource.getPluginId())));
    ...
    public Mono<DatasourceTestResult> testDatasource(Datasource datasource) {
        ...
        if (datasource.getId() != null && datasource.getDatasourceConfiguration() != null &&
            ...
            datasourceMono = getById(datasource.getId())
                    .map(datasource1 -> {
                        BizbrainzBeanUtils.copyNestedNonNullProperties(datasource, datasource1);
                        return datasource1;
                    })
    ...
    public Flux<Datasource> get(MultiValueMap<String, String> params) {
        ...
        return Flux.error(new BizbrainzException(BizbrainzError.INVALID_PARAMETER, FieldName.WORKSPACE_ID));
    }
    ...
    public Mono<Datasource> archiveById(String id) {
        return repository
                ...
                .flatMap(objects -> {
                    ...
                    if (actionsCount > 0) {
                        return Mono.error(new BizbrainzException(BizbrainzError.DATASOURCE_HAS_ACTIONS, actionsCount));
                    }
