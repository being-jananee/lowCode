...
import com.bizBrainz.server.domains.QTenant;
import com.bizBrainz.server.domains.QTheme;
import com.bizBrainz.server.domains.QUser;
...
import static com.bizBrainz.external.helpers.BizbrainzBeanUtils.copyNestedNonNullProperties;
import static com.bizBrainz.server.acl.AclPermission.ASSIGN_PERMISSION_GROUPS;
import static com.bizBrainz.server.acl.AclPermission.MANAGE_INSTANCE_CONFIGURATION;
import static com.bizBrainz.server.acl.AclPermission.MANAGE_INSTANCE_ENV;
import static com.bizBrainz.server.acl.AclPermission.MANAGE_USERS;
import static com.bizBrainz.server.acl.AclPermission.READ_INSTANCE_CONFIGURATION;
import static com.bizBrainz.server.acl.AclPermission.READ_PERMISSION_GROUPS;
...
import static com.bizBrainz.server.acl.AclPermission.RESET_PASSWORD_USERS;
import static com.bizBrainz.server.constants.EnvVariables.BIZBRAINZ_ADMIN_EMAILS;
import static com.bizBrainz.server.constants.FieldName.DEFAULT_PERMISSION_GROUP;
import static com.bizBrainz.server.constants.FieldName.PERMISSION_GROUP_ID;
import static com.bizBrainz.server.helpers.CollectionUtils.findSymmetricDiff;
import static com.bizBrainz.server.migrations.DatabaseChangelog.dropIndexIfExists;
import static com.bizBrainz.server.migrations.DatabaseChangelog.ensureIndexes;
import static com.bizBrainz.server.migrations.DatabaseChangelog.getUpdatedDynamicBindingPathList;
import static com.bizBrainz.server.migrations.DatabaseChangelog.installPluginToAllWorkspaces;
import static com.bizBrainz.server.migrations.DatabaseChangelog.makeIndex;
import static com.bizBrainz.server.migrations.MigrationHelperMethods.evictPermissionCacheForUsers;
...
public class DatabaseChangelog2 {
    ...
    public static void migrateFirestoreActionsFormData(NewAction uqiAction) {
        ...
        if (unpublishedFormData != null) {
            ...
            if (!(command instanceof String)) {
                throw new BizbrainzException(BizbrainzError.MIGRATION_ERROR);
            }
    ...
    public void updateSequenceNamesFromOrganizationToWorkspace(MongockTemplate mongockTemplate) {
        for (Sequence sequence : mongockTemplate.findAll(Sequence.class)) {
            ...
            if (!newName.equals(oldName)) {
                //Using strings in the field names instead of QSequence becauce Sequence is not a BizbrainzDomain
    ...
    private Set<PermissionGroup> generatePermissionsForDefaultPermissionGroups(MongockTemplate mongockTemplate, PolicyUtils policyUtils, Set<PermissionGroup> permissionGroups, Workspace workspace, Map<String, String> userIdForEmail, Set<String> validUserIds) {
        ...
        Set<Permission> workspacePermissions = BizbrainzRole.ORGANIZATION_ADMIN
                .getPermissions()
                .stream()
        ...
        Set<String> adminUserIds = userRoles
                .stream()
                .filter(userRole -> userRole.getRole().equals(BizbrainzRole.ORGANIZATION_ADMIN))
        ...
        workspacePermissions = BizbrainzRole.ORGANIZATION_DEVELOPER
                .getPermissions()
                .stream()
                .filter(aclPermission -> aclPermission.getEntity().equals(Workspace.class))
                .map(aclPermission -> new Permission(workspace.getId(), aclPermission))
        ...
        Set<String> developerUserIds = userRoles
                .stream()
                .filter(userRole -> userRole.getRole().equals(BizbrainzRole.ORGANIZATION_DEVELOPER))
                .map(UserRole::getUserId)
                .collect(Collectors.toSet());
        ...
        workspacePermissions = BizbrainzRole.ORGANIZATION_VIEWER
                .getPermissions()
                .stream()
                .filter(aclPermission -> aclPermission.getEntity().equals(Workspace.class))
                .map(aclPermission -> new Permission(workspace.getId(), aclPermission))
        ...
        Set<String> viewerUserIds = userRoles
                .stream()
                .filter(userRole -> userRole.getRole().equals(BizbrainzRole.ORGANIZATION_VIEWER))
                .map(UserRole::getUserId)
                .collect(Collectors.toSet());
    ...
    public void updateSuperUsers(MongockTemplate mongockTemplate, CacheableRepositoryHelper cacheableRepositoryHelper) {
        ...
        String adminEmailsStr = System.getenv(String.valueOf(BIZBRAINZ_ADMIN_EMAILS));
    ...
    public void addGraphQLPlugin(MongockTemplate mongoTemplate) {
        ...
        plugin.setIconLocation("https://s3.us-east-2.amazonaws.com/assets.bizBrainz.com/logo/graphql.svg");
        plugin.setDocumentationLink("https://docs.bizBrainz.com/reference/datasources/querying-graphql-db");
