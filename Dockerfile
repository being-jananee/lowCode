...
LABEL maintainer="tech@bizBrainz.com"

# Set workdir to /opt/bizBrainz
WORKDIR /opt/bizBrainz

# The env variables are needed for Bizbrainz server to correctly handle non-roman scripts like Arabic.
ENV LANG C.UTF-8
ENV LC_ALL C.UTF-8
...
VOLUME [ "/bizBrainz-stacks" ]

# ------------------------------------------------------------------------
# Add backend server - Application Layer
ARG JAR_FILE=./app/server/dist/server-*.jar
ARG PLUGIN_JARS=./app/server/dist/plugins/*.jar
ARG BIZBRAINZ_SEGMENT_CE_KEY
ENV BIZBRAINZ_SEGMENT_CE_KEY=${BIZBRAINZ_SEGMENT_CE_KEY}
...
# Update path to load bizBrainz utils tool as default
ENV PATH /opt/bizBrainz/utils/node_modules/.bin:$PATH
...
EXPOSE 80
EXPOSE 443
ENTRYPOINT [ "/opt/bizBrainz/entrypoint.sh" ]
HEALTHCHECK --interval=15s --timeout=15s --start-period=45s CMD "/opt/bizBrainz/healthcheck.sh"
