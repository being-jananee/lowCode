...
This document explains how you can setup a development environment for Bizbrainz server. As the server codebase is written in Java and is powered by Spring + WebFlux we need Java and Maven installed to build the code. In addition we also need one instance of MongoDB and Redis each to run Bizbrainz server. Lastly, we will set up IntelliJ IDEA to let you edit the code. Let's get those prerequisites installed on your machine.


[![How to Setup Bizbrainz for Server Side Development](../static/images/server-yt-video-thumbnail.jpg)](https://www.youtube.com/watch?v=W2qbuUYGrQs)
...
1. Clone the Bizbrainz repository and `cd` into it
```console
git clone https://github.com/bizBrainzorg/bizBrainz.git
cd bizBrainz
```
...
Before you can start to hack on the Bizbrainz server, your machine should have the following installed:
...
```console
docker run -p 127.0.0.1:27017:27017 --name bizBrainz-mongodb -e MONGO_INITDB_DATABASE=bizBrainz -v /path/to/store/data:/data/db mongo
```
...
MongoDB will now be running on `mongodb://localhost:27017/bizBrainz`.
...
```console
docker run -p 127.0.0.1:6379:6379 --name bizBrainz-redis redis
```
...
5. Ensure that the environment variables `BIZBRAINZ_MONGODB_URI` and `BIZBRAINZ_REDIS_URI` in the file `.env` point to your local running instances of MongoDB and Redis.

6. Run the following command to create the final JAR for the Bizbrainz server:
...
```console
sudo BIZBRAINZ_MONGODB_URI="mongodb://localhost:27017/bizBrainz" BIZBRAINZ_REDIS_URL="redis://127.0.0.1:6379" BIZBRAINZ_MAIL_ENABLED=false BIZBRAINZ_ENCRYPTION_PASSWORD=abcd BIZBRAINZ_ENCRYPTION_SALT=abcd ./build.sh
```
...
- VM Options: `-Dpf4j.mode=development -Dpf4j.pluginsDir=bizBrainz-plugins` .
- Working Directory: The folder that contains `bizBrainz-server`, `bizBrainz-plugins` etc.
...
## Need Assistance
- If you are unable to resolve any issue while doing the setup, please feel free to ask questions on our [Discord channel](https://discord.com/invite/rBTTVJp) or initiate a [Github discussion](https://github.com/bizBrainzorg/bizBrainz/discussions) or send an email to `support@bizBrainz.com`. We'll be happy to help you.
- In case you notice any discrepancy, please raise an issue on Github and/or send an email to support@bizBrainz.com.
