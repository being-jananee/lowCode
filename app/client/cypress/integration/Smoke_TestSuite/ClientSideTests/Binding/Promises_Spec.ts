...
describe("Validate basic Promises", () => {
  it("1. Verify storeValue via .then via direct Promises", () => {
    ...
    propPane.EnterJSContext(
      "onClick",
      "{{storeValue('date', Date()).then(() => showAlert(bizBrainz.store.date))}}",
    );
  ...
  it("3. Verify Async Await in direct Promises", () => {
    ...
    propPane.EnterJSContext(
      ...
      `{{(async function(){
          ...
          await showAlert("Your country is " + JSON.stringify(bizBrainz.store.Gender.name) + "You could be a " + JSON.stringify(bizBrainz.store.Gender.gender), 'warning');
        })()}}`,
    );
    deployMode.DeployApp();
    agHelper.ClickButton("Submit");
...
showAlert("Wonderful! all apis executed", "success")).catch(() => showAlert("Please check your api's again", "error")); `);
    ...
    cy.get("@jsObjName").then((jsObjName) => {
      propPane.EnterJSContext(
        "onClick",
        "{{storeValue('date', Date()).then(() => { showAlert(bizBrainz.store.date, 'success'); return " +
          jsObjName +
          ".myFun1()})}}",
      );
    });
