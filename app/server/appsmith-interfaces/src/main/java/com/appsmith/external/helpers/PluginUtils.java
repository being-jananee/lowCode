...
import static com.bizBrainz.external.constants.CommonFieldName.CHILDREN;
import static com.bizBrainz.external.constants.CommonFieldName.CONDITION;
import static com.bizBrainz.external.constants.CommonFieldName.KEY;
import static com.bizBrainz.external.constants.CommonFieldName.VALUE;
...
public class PluginUtils {
    ...
    public static Set<String> getHintMessageForLocalhostUrl(DatasourceConfiguration datasourceConfiguration) {
        ...
        if (datasourceConfiguration != null) {
            ...
            if(usingLocalhostUrl) {
                message.add("You may not be able to access your localhost if Bizbrainz is running inside a docker " +
                        "container or on the cloud. To enable access to your localhost you may use ngrok to expose " +
                        "your local endpoint to the internet. Please check out Bizbrainz's documentation to understand more" +
                        ".");
            }
        }
    ...
    public static Condition parseWhereClause(Map<String, Object> whereClause) {
        ...
        try {
            ...
            throw new BizbrainzPluginException(BizbrainzPluginError.PLUGIN_UQI_WHERE_CONDITION_UNKNOWN, unparsedOperator);
        }
    ...
    public static void setValueSafelyInPropertyList(List<Property> properties, int index, Object value) throws BizbrainzPluginException {
        if (properties == null) {
            throw new BizbrainzPluginException(
                    BizbrainzPluginError.PLUGIN_ERROR,
                    "Bizbrainz server encountered an unexpected error: property list is null. Please reach out to " +
        ...
        if (index < 0 || index > properties.size() - 1) {
            throw new BizbrainzPluginException(
                    BizbrainzPluginError.PLUGIN_ERROR,
                    "Bizbrainz server encountered an unexpected error: index value out or range: index: " + index +
