...
import static com.bizBrainz.server.acl.AclPermission.READ_DATASOURCES;
...
import static com.bizBrainz.server.acl.AclPermission.UNASSIGN_PERMISSION_GROUPS;
...
import static com.bizBrainz.server.constants.FieldName.DEVELOPER;
import static com.bizBrainz.server.constants.FieldName.VIEWER;
...
public class WorkspaceServiceTest {
    ...
    private static String origin = "http://bizBrainz-local.test";
    ...
    public void nullCreateWorkspace() {
        ...
        StepVerifier.create(workspaceResponse)
                .expectErrorMatches(throwable -> throwable instanceof BizbrainzException &&
                        throwable.getMessage().equals(BizbrainzError.INVALID_PARAMETER.getMessage(FieldName.WORKSPACE)))
                .verify();
    }
    ...
    public void nullName() {
        ...
        StepVerifier.create(workspaceResponse)
                .expectErrorMatches(throwable -> throwable instanceof BizbrainzException &&
                        throwable.getMessage().equals(BizbrainzError.INVALID_PARAMETER.getMessage(FieldName.NAME)))
                .verify();
    }
    ...
    public void getWorkspaceNullId() {
        ...
        StepVerifier.create(workspaceMono)
                .expectErrorMatches(throwable -> throwable instanceof BizbrainzException &&
                        throwable.getMessage().equals(BizbrainzError.INVALID_PARAMETER.getMessage(FieldName.ID)))
                .verify();
    }
                    ...
                    return workspaceService.update(t.getT1().getId(), changes)
                            .zipWhen(updatedWorkspace ->
                                    workspaceRepository.findById(t.getT2().getId(), READ_WORKSPACES)
                                            .switchIfEmpty(Mono.error(new BizbrainzException(BizbrainzError.NO_RESOURCE_FOUND, FieldName.WORKSPACE, t.getT2().getId())))
                            );
                });
    ...
    public void validUpdateWorkspaceInvalidEmail() {
        ...
        for (String invalidEmail : invalidEmails) {
            ...
            StepVerifier.create(updateWorkspace)
                    .expectErrorMatches(throwable -> throwable instanceof BizbrainzException &&
                            throwable.getMessage().equals(BizbrainzError.INVALID_PARAMETER.getMessage(FieldName.EMAIL)))
                    .verify();
        }
    }
    ...
    public void validUpdateWorkspaceInvalidWebsite() {
        ...
        for (String invalidWebsite : invalidWebsites) {
            ...
            StepVerifier.create(updateWorkspace)
                    .expectErrorMatches(throwable -> throwable instanceof BizbrainzException &&
                            throwable.getMessage().equals(BizbrainzError.INVALID_PARAMETER.getMessage(FieldName.WEBSITE)))
                    .verify();
        }
    }
    ...
    public void addUserToWorkspaceAsAdminAndCheckApplicationAndDatasourcePermissions() {
        ...
        Mono<Application> readApplicationByNameMono = applicationService.findByName("User Management Admin Test Application",
                        READ_APPLICATIONS)
                .switchIfEmpty(Mono.error(new BizbrainzException(BizbrainzError.NO_RESOURCE_FOUND, "application by name")));

        Mono<Workspace> readWorkspaceByNameMono = workspaceRepository.findByName("Member Management Admin Test Workspace")
                .switchIfEmpty(Mono.error(new BizbrainzException(BizbrainzError.NO_RESOURCE_FOUND, "workspace by name")));
        ...
        Mono<Datasource> readDatasourceByNameMono = workspaceMono.flatMap(workspace1 ->
                datasourceRepository.findByNameAndWorkspaceId("test datasource", workspace1.getId(), READ_DATASOURCES)
                        .switchIfEmpty(Mono.error(new BizbrainzException(BizbrainzError.NO_RESOURCE_FOUND, "Datasource")))
        );
    ...
    public void inviteRolesGivenAdministrator() {
        Set<BizbrainzRole> roles = roleGraph.generateHierarchicalRoles("Administrator");
        BizbrainzRole administratorRole = BizbrainzRole.generateBizbrainzRoleFromName("Administrator");
        BizbrainzRole developerRole = BizbrainzRole.generateBizbrainzRoleFromName("Developer");
        BizbrainzRole viewerRole = BizbrainzRole.generateBizbrainzRoleFromName("App Viewer");

        StepVerifier.create(Mono.just(roles))
                .assertNext(bizBrainzRoles -> {
                    assertThat(bizBrainzRoles).isNotNull();
                    assertThat(bizBrainzRoles).containsAll(Set.of(administratorRole, developerRole, viewerRole));
                })
                .verifyComplete();
    }
    ...
    public void inviteRolesGivenDeveloper() {
        Set<BizbrainzRole> roles = roleGraph.generateHierarchicalRoles("Developer");
        BizbrainzRole developerRole = BizbrainzRole.generateBizbrainzRoleFromName("Developer");
        BizbrainzRole viewerRole = BizbrainzRole.generateBizbrainzRoleFromName("App Viewer");
        ...
        StepVerifier.create(Mono.just(roles))
                .assertNext(bizBrainzRoles -> {
                    assertThat(bizBrainzRoles).isNotNull();
                    assertThat(bizBrainzRoles).containsAll(Set.of(developerRole, viewerRole));
                })
                .verifyComplete();
    }
    ...
    public void inviteRolesGivenViewer() {
        Set<BizbrainzRole> roles = roleGraph.generateHierarchicalRoles("App Viewer");
        BizbrainzRole viewerRole = BizbrainzRole.generateBizbrainzRoleFromName("App Viewer");

        StepVerifier.create(Mono.just(roles))
                .assertNext(bizBrainzRoles -> {
                    assertThat(bizBrainzRoles).isNotNull();
                    assertThat(bizBrainzRoles).hasSize(1);
                    assertThat(bizBrainzRoles).containsAll(Set.of(viewerRole));
                })
                .verifyComplete();
    }
    ...
    public void uploadWorkspaceLogo_nullFilePart() throws IOException {
        ...
        StepVerifier.create(resultMono)
                .expectErrorMatches(throwable -> throwable instanceof BizbrainzException &&
                        throwable.getMessage().equals(BizbrainzError.VALIDATION_FAILURE.getMessage("Please upload a valid image.")))
                .verify();
    }
    ...
    public void uploadWorkspaceLogo_largeFilePart() throws IOException {
        ...
        StepVerifier.create(resultMono)
                .expectErrorMatches(throwable -> throwable instanceof BizbrainzException &&
                        throwable.getMessage().equals(BizbrainzError.PAYLOAD_TOO_LARGE.getMessage(Constraint.WORKSPACE_LOGO_SIZE_KB)))
                .verify();
    }
    ...
    public void testDeleteLogo_invalidWorkspace() {
        ...
        StepVerifier.create(deleteLogo)
                .expectErrorMatches(throwable -> throwable instanceof BizbrainzException &&
                        throwable.getMessage().equals(BizbrainzError.NO_RESOURCE_FOUND.getMessage(FieldName.WORKSPACE, "")))
                .verify();
    }
    ...
    public void delete_WhenWorkspaceHasApp_ThrowsException() {
        ...
        StepVerifier
                ...
                .expectErrorMessage(BizbrainzError.UNSUPPORTED_OPERATION.getMessage())
                .verify();
    }
    ...
    public void delete_WithoutManagePermission_ThrowsException() {
        ...
        StepVerifier
                .create(deleteWorkspaceMono)
                .expectError(BizbrainzException.class)
                .verify();
    }
    ...
    public void update_WhenNameIsNotPresent_SlugIsNotGenerated() {
        ...
        Mono<Workspace> workspaceMono = workspaceService.create(workspace)
                .flatMap(savedWorkspace -> {
                    ...
                    workspaceDto.setWebsite("https://bizBrainz.com");
