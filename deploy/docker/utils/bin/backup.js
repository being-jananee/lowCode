...
async function run() {
  ...
  try {
    ...
    console.log('Available free space at /bizBrainz-stacks');
    const availSpaceInBytes = parseInt(shell.exec('df --output=avail -B 1 /bizBrainz-stacks | tail -n 1'), 10);
    console.log('\n');

    if (availSpaceInBytes < Constants.MIN_REQUIRED_DISK_SPACE_IN_BYTES) {
      throw new Error('Not enough space avaliable at /bizBrainz-stacks. Please ensure availability of atleast 5GB to backup successfully.');
    }

    const backupRootPath = await fsPromises.mkdtemp(path.join(os.tmpdir(), 'bizBrainzctl-backup-'));
    const backupContentsPath = backupRootPath + '/bizBrainz-backup-' + timestamp;
...
async function exportDatabase(destFolder) {
  ...
  await utils.execCommand(['mongodump', `--uri=${process.env.BIZBRAINZ_MONGODB_URI}`, `--archive=${destFolder}/mongodb-data.gz`, '--gzip']);
...
async function createGitStorageArchive(destFolder) {
  ...
  let gitRoot = process.env.BIZBRAINZ_GIT_ROOT;
  if (gitRoot == null || gitRoot === '') {
    gitRoot = '/bizBrainz-stacks/git-storage';
  }
...
async function createManifestFile(path) {
  ...
  const manifest_data = { "bizBrainzVersion": version }
...
async function exportDockerEnvFile(destFolder) {
  ...
  const content = await fsPromises.readFile('/bizBrainz-stacks/configuration/docker.env', { encoding: 'utf8' });
  const output_lines = []
  content.split(/\r?\n/).forEach(line => {
    if (!line.startsWith("BIZBRAINZ_ENCRYPTION")) {
  ...
  console.log('!!! Please ensure you have saved the BIZBRAINZ_ENCRYPTION_SALT and BIZBRAINZ_ENCRYPTION_PASSWORD variables from the docker.env file because those values are not included in the backup export.');
  console.log('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!');
}
...
async function createFinalArchive(destFolder, timestamp) {
  ...
  const archive = `${Constants.BACKUP_PATH}/bizBrainz-backup-${timestamp}.tar.gz`;
...
async function postBackupCleanup(){
  ...
  let backupArchivesLimit = process.env.BIZBRAINZ_BACKUP_ARCHIVE_LIMIT;
