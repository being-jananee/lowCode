        ...
        return datasourceService.getById(datasourceId)
                ...
                .onErrorMap(e -> {
                    if (!(e instanceof BizbrainzPluginException)) {
                        return new BizbrainzPluginException(BizbrainzPluginError.PLUGIN_GET_STRUCTURE_ERROR, e.getMessage());
                    }
        ...
        return pluginExecutorHelper
                ...
                .onErrorMap(
                        ...
                        error -> new BizbrainzPluginException(
                                BizbrainzPluginError.PLUGIN_GET_STRUCTURE_TIMEOUT_ERROR,
                                "Bizbrainz server timed out when fetching structure. Please reach out to bizBrainz " +
                ...
                .onErrorMap(
                        ...
                        error -> new BizbrainzPluginException(
                                ...
                                "Bizbrainz server found a secondary stale connection. Please reach out to bizBrainz " +
                                        "customer support to resolve this."
                        )
                )
                .onErrorMap(
    ...
    public Mono<ActionExecutionResult> getDatasourceMetadata(String datasourceId, List<Property> pluginSpecifiedTemplates) {
        ...
        Mono<Datasource> datasourceMono = datasourceService.findById(datasourceId, AclPermission.MANAGE_DATASOURCES)
                .switchIfEmpty(Mono.error(new BizbrainzException(
                        BizbrainzError.ACL_NO_RESOURCE_FOUND, FieldName.DATASOURCE, datasourceId
                )))
        ...
        return datasourceMono.flatMap(datasource -> {
            ...
            if (auth == null) {
                ...
                return Mono.error(new BizbrainzException(
                        BizbrainzError.INVALID_DATASOURCE,
                        datasource.getName(),
