...
import com.bizBrainz.external.dtos.MultipartFormDataDTO;
...
import com.bizBrainz.external.helpers.DataTypeStringUtils;
...
import com.bizBrainz.external.services.FilterDataService;
...
import static com.bizBrainz.external.constants.ActionConstants.ACTION_CONFIGURATION_BODY;
import static com.bizBrainz.external.constants.ActionConstants.ACTION_CONFIGURATION_PATH;
import static com.bizBrainz.external.helpers.PluginUtils.OBJECT_TYPE;
import static com.bizBrainz.external.helpers.PluginUtils.STRING_TYPE;
import static com.bizBrainz.external.helpers.PluginUtils.getDataValueSafelyFromFormData;
import static com.bizBrainz.external.helpers.PluginUtils.parseList;
import static com.bizBrainz.external.helpers.PluginUtils.parseWhereClause;
import static com.bizBrainz.external.helpers.PluginUtils.setDataValueSafelyInFormData;
...
public class AmazonS3Plugin extends BasePlugin {
    ...
    public static class S3PluginExecutor implements PluginExecutor<AmazonS3>, SmartSubstitutionInterface {
         ...
         */
        ArrayList<String> getFilenamesFromObjectListing(ObjectListing objectListing) throws BizbrainzPluginException {
            if (objectListing == null) {
                throw new BizbrainzPluginException(
                        BizbrainzPluginError.PLUGIN_ERROR,
                        "Bizbrainz server has encountered an unexpected error when fetching file " +
                                "content from AWS S3 server. Please reach out to Bizbrainz customer support to resolve this"
                );
            }
        ...
        ArrayList<String> listAllFilesInBucket(AmazonS3 connection,
                                               String bucketName,
                                               String prefix) throws BizbrainzPluginException {
            if (connection == null) {
                throw new BizbrainzPluginException(
                        BizbrainzPluginError.PLUGIN_ERROR,
                        "Bizbrainz server has encountered an unexpected error when establishing " +
                                "connection with AWS S3 server. Please reach out to Bizbrainz customer support to resolve this."
                );
            }
            ...
            if (bucketName == null) {
                ...
                throw new BizbrainzPluginException(
                        BizbrainzPluginError.PLUGIN_ERROR,
                        "Bizbrainz has encountered an unexpected error when getting bucket name. Please reach out to " +
            ...
            if (prefix == null) {
                ...
                throw new BizbrainzPluginException(
                        BizbrainzPluginError.PLUGIN_ERROR,
                        "Bizbrainz has encountered an unexpected error when getting path prefix. Please reach out to " +
                                "Bizbrainz customer support to resolve this."
                );
            }
        ...
        String uploadFileFromBody(AmazonS3 connection,
                                  ...
                                  Date expiryDateTime)
                throws InterruptedException, BizbrainzPluginException {
            ...
            if (listOfUrls.size() != 1) {
                throw new BizbrainzPluginException(
                        BizbrainzPluginError.PLUGIN_ERROR,
                        "Bizbrainz has encountered an unexpected error while fetching url from AmazonS3 after file " +
                                "creation. Please reach out to Bizbrainz customer support to resolve this."
                );
            }
        ...
        public Mono<ActionExecutionResult> executeParameterized(AmazonS3 connection,
            ...
            } catch (BizbrainzPluginException e) {
                // Initializing object for error condition
                ActionExecutionResult errorResult = new ActionExecutionResult();
                errorResult.setStatusCode(BizbrainzPluginError.PLUGIN_ERROR.getAppErrorCode().toString());
        ...
        private Mono<ActionExecutionResult> executeCommon(AmazonS3 connection,
            ...
            return Mono.fromCallable(() -> {
                            ...
                            case LIST:
                                ...
                                if (isSignedUrl) {
                                    ...
                                    if (listOfFiles.size() != listOfSignedUrls.size()) {
                                        return Mono.error(new BizbrainzPluginException(
                                                BizbrainzPluginError.PLUGIN_ERROR,
                                                "Bizbrainz server has encountered an unexpected error when getting " +
                                                        "list of files from AWS S3 server. Please reach out to Bizbrainz customer " +
                    ...
                    // Transform AmazonS3Exception and AmazonServiceException to BizbrainzPluginException
                    .onErrorResume(e -> {
                        if (e instanceof AmazonS3Exception || e instanceof AmazonServiceException) {
                            return Mono.error(new BizbrainzPluginException(e, BizbrainzPluginError.PLUGIN_ERROR, e.getMessage()));
                        }
                        return Mono.error(e);
                    })
                    .onErrorResume(e -> {
        ...
        private void deleteMultipleObjects(AmazonS3 connection, String bucketName, String path) throws BizbrainzPluginException {
            ...
            try {
                ...
                throw new BizbrainzPluginException(
                        BizbrainzPluginError.PLUGIN_EXECUTE_ARGUMENT_ERROR,
                        "Bizbrainz server failed to parse the list of files. Please provide the list of files in the " +
        ...
        public Mono<AmazonS3> datasourceCreate(DatasourceConfiguration datasourceConfiguration) {
            ...
            try {
                ...
                return Mono.error(
                        new BizbrainzPluginException(
                                BizbrainzPluginError.PLUGIN_ERROR,
                                "Bizbrainz server has failed to load AWS S3 driver class. Please reach out to Bizbrainz " +
            ...
            return Mono.fromCallable(() -> getS3ClientBuilder(datasourceConfiguration).build())
                    ...
                    .onErrorResume(e -> {
                                ...
                                return Mono.error(
                                        new BizbrainzPluginException(
                                                BizbrainzPluginError.PLUGIN_ERROR,
                                                "Bizbrainz server has encountered an error when " +
        ...
        public Set<String> validateDatasource(DatasourceConfiguration datasourceConfiguration) {
            ...
            if (properties == null
                ...
                invalids.add("Bizbrainz has failed to fetch the 'S3 Service Provider' field properties. Please " +
                        "reach out to Bizbrainz customer support to resolve this.");
            }
        ...
        public Mono<DatasourceStructure> getStructure(AmazonS3 connection, DatasourceConfiguration datasourceConfiguration) {
            ...
            return Mono.fromSupplier(() -> {
                        ...
                        try {
                            ...
                            throw new BizbrainzPluginException(
                                    BizbrainzPluginError.PLUGIN_GET_STRUCTURE_ERROR,
                                    "Bizbrainz server has failed to fetch list of buckets from database. Please check if " +
