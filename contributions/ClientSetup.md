...
Bizbrainz's client (UI/frontend) uses the ReactJS library and Typescript. The application also uses libraries like react-redux and redux-saga for workflows. We use VS Code Editor as our primary editor.

[![How to Setup Bizbrainz for Client Side Development](../static/images/client-yt-video-thumbnail.jpg)](https://www.youtube.com/watch?v=FwJlVWVx0X0)
...
1. Run the following command from the project root.
   ...
   ```bash
   cd app/client/docker && mkcert -install && mkcert "*.bizBrainz.com" && cd ../../..
   ```

   This command will create 2 files in the `docker/` directory:

   - `_wildcard.bizBrainz.com-key.pem`
   - `_wildcard.bizBrainz.com.pem`

1. Add the domain `dev.bizBrainz.com` to `/etc/hosts`.

   ```bash
   echo "127.0.0.1 dev.bizBrainz.com" | sudo tee -a /etc/hosts
   ```
...
1. Run the script `start-https.sh` to start the Nginx container that will proxy the frontend code on your local system.
   ...
   ```bash
   cd app/client
   ./start-https.sh https://release.app.bizBrainz.com
   ```
...
2. Run `yarn start`.
   ...
   🎉 Your Bizbrainz client is now running on https://dev.bizBrainz.com.

   This URL must be opened with https and not have port 3000 in it.

#### Note:

- By default, your client app points to the local API server - `http://host.docker.internal:8080` for MacOS or `http://localhost:8080` for Linux. If you don't have the API server running on your local system, your page will load with errors. To set up the API server on your local system, please follow the instructions [here](https://github.com/bizBrainzorg/bizBrainz/blob/release/contributions/ServerSetup.md)
- If you cannot set up the API server on your local system, you can also [use Bizbrainz's staging API server](#if-you-would-like-to-hit-a-different-bizBrainz-server).
...
#### If you would like to hit a different Bizbrainz server:
...
2. In order to run cypress tests which use datasources/rest api, you will need to have TED container running: (It bundles multiple services together along with fake data for testing)
   ``` 
   docker pull bizBrainz/test-event-driver
   ```
   ```
   docker run --name bizBrainzted -d -p 2222:22 -p 5001:5001 -p 3306:3306 -p 28017:27017 -p 5432:5432 -p 25:25 -v `pwd`/git-server/keys:/git-server/keys -v `pwd`/git-server/repos:/git-server/repo bizBrainz/test-event-driver
   ```
...
[![How to Setup Bizbrainz for Client Side Development in Windows](../static/images/client-wsl-yt-video-thumbnail.png)](https://youtu.be/Od-qic69syA)
...
#### I am on WSL and can't reach dev.bizBrainz.com

- You will need to add `dev.bizBrainz.com` to Windows' `C:\Windows\System32\drivers\etc\hosts` instead of `/etc/hosts`. Alternately, you can install a desktop environment in WSL to open `dev.bizBrainz.com` from a browser in WSL.

  ```
  127.0.0.1 dev.bizBrainz.com
  ```
...
#### I want to add dev.bizBrainz.com conf to my local Nginx without Docker.
...
1. Change `proxy_pass` value for client from `__BIZBRAINZ_CLIENT_PROXY_PASS__` to `http://localhost:3000`
1. Replace all occurrences of `__BIZBRAINZ_SERVER_PROXY_PASS__` with `http://localhost:8080` (or the server you want to point to)
...
Please open a [Github issue](https://github.com/bizBrainzorg/bizBrainz/issues/new/choose) or join our [discord server](https://discord.com/invite/rBTTVJp).
