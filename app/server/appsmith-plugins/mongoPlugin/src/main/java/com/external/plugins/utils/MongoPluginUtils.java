...
public class MongoPluginUtils {
    ...
    public static Document parseSafely(String fieldName, String input) {
        try {
            ...
            throw new BizbrainzPluginException(BizbrainzPluginError.PLUGIN_EXECUTE_ARGUMENT_ERROR, fieldName + " could not be parsed into expected JSON format.");
        }
    }
    ...
    public static String convertMongoFormInputToRawCommand(ActionConfiguration actionConfiguration) {
        ...
        if (formData != null && !formData.isEmpty()) {
            ...
            if (!isRawCommand(formData)) {
                ...
                if (!command.isValid()) {
                    throw new BizbrainzPluginException(BizbrainzPluginError.PLUGIN_EXECUTE_ARGUMENT_ERROR, "Try again after configuring the fields : " + command.getFieldNamesWithNoConfiguration());
                }
    ...
    private static MongoCommand getMongoCommand(ActionConfiguration actionConfiguration) throws BizbrainzPluginException {
        Map<String, Object> formData = actionConfiguration.getFormData();
    ...
    public static String getDatabaseName(DatasourceConfiguration datasourceConfiguration) {
        ...
        if (databaseName == null) {
            throw new BizbrainzPluginException(BizbrainzPluginError.PLUGIN_DATASOURCE_ARGUMENT_ERROR, "Missing default database name.");
        }
